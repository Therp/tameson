...
class PurchaseOrderLineCsvImportImport(models.TransientModel):
    ...
    def _get_default_purchase_order(self):
        ...
        if purchase_order_id and purchase_order_id.state != "draft":
            raise UserError(
                _("Only orders in draft state can be used with the .csv import!")
            )
        return self._context.get("active_id", False)

    purchase_order_id = fields.Many2one(
        "purchase.order", required=True, default=_get_default_purchase_order
    )
    csv_file = fields.Binary(string="File", help="Must be in .csv format.")
    filename = fields.Char(string="Filename")
    line_ids = fields.One2many(
        "purchase.orderline.csv.import.line", "wizard_id", string="Lines to import"
    )
    state = fields.Selection(
        string="State",
        selection=[("draft", "Draft"), ("loaded", "Loaded"), ("imported", "Imported")],
        default="draft",
    )
    ...
    def action_load(self):
        if self.filename and self.filename[-3:] == "csv":
            ...
            for line in data:
                ...
                sku = line[1]
                product = (
                    product_product_obj.search([("default_code", "=", sku)], limit=1)
                    or False
                )
                if product:
                    ...
                    raise UserError(
                        _(
                            "Line #"
                            + str(rowno)
                            + " does not have a valid product SKU: "
                            + sku
                            + ", please correct it!"
                        )
                    )
    ...
    def action_import(self):
        ...
        if self.line_ids:
                ...
                order_lines.append(
                    (
                        ...
                        0,
                        {
                            ...
                            and line.product_id.uom_po_id.id
            ...
            try:
                self.purchase_order_id.write({"order_line": order_lines})
            except Exception as e:
                raise UserError(
                    _("Purchase order lines were not imported, error: \n\n" + str(e))
                )
