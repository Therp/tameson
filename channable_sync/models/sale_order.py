...
class SaleOrder(models.Model):
    ...
    channable_to_update_cancel = fields.Boolean(
        string="Needs To Be Cancelled in Channable", default=False, copy=False
    )
    channable_to_update_shipped = fields.Boolean(
        string="Needs To Be Updated in Channable", default=False, copy=False
    )
    channable_refused_cancellation = fields.Boolean(
        string="Refused Cancellation in Channable", default=False, copy=False
    )
    ...
    def sync_orders_with_channable(self):
        ...
        limit_size = (
            ...
            .sudo()
            .get_param("channable_sync.limit_orders_request_size", default=20)
        )
        try:
            limit_size = int(limit_size)
        except Exception as e:
            _logger.warning(
                "Error in converting the limit orders request size config parameter: %s"
                % str(e)
            )
            limit_size = 20
        orders_last_n_days_sync = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("channable_sync.orders_last_n_days_sync", default=0)
        )
        try:
            orders_last_n_days_sync = int(orders_last_n_days_sync)
        except Exception as e:
            _logger.warning(
                "Error in converting the number of days sync from the Channable order API: %s"
                % str(e)
            )
        ...
        if orders_last_n_days_sync:
            start_date = fields.Date.to_string(
                datetime.date.today()
                - relativedelta.relativedelta(days=orders_last_n_days_sync)
            )
        ...
        if not all(
            [
                self.company_id.channable_orders_shipped,
                self.company_id.channable_orders_not_shipped,
                self.company_id.channable_orders_cancelled,
                self.company_id.channable_orders_waiting,
            ]
        ):
            ...
            if states_to_fetch:
                payload.update({"status": states_to_fetch})

        response = self.env.user.company_id.channable_request(
            method="GET", endpoint="/orders", payload=payload, timeout=600
        )
        all_orders = response.get("orders", [])
        while all_orders:
            for order in all_orders:
                existing_order = self.search(
                    [("channable_order_id", "=", order.get("id"))], limit=1
                )
                if not existing_order:
                    ...
                    if (
                        channable_status == "cancelled"
                        and existing_order.state != "cancel"
                    ):
                        if any(
                            pick.state == "done"
                            for pick in existing_order.mapped("picking_ids")
                        ):
        ...
        msg = "Channable sync: successfully imported %s new orders." % str(
    ...
    def import_channable_order(self, order_body):
        ...
        if existing_order:
            ...
            if channable_status == "cancelled" and existing_order.state != "cancel":
                if any(
                    ...
                    return (
                        "Channable order import: order canceled in Channable, but unable to be cancelled in Odoo: %s"
                        % str(existing_order.name)
                    )
                existing_order.action_cancel()
                existing_order.write({"channable_order_status": "cancelled"})
                return (
                    "Channable order import: order cancelled after being cancelled in Channable: %s"
                    % str(existing_order.name)
                )
            else:
                return "Channable order import: existing order, skipping: %s" % str(
                    existing_order.name
                )
        ...
        partner, partner_invoice, partner_shipping = self.get_channable_customer(
            email, customer, billing, shipping, currency
        )
        if not partner:
        ...
        if pricelist.currency_id != res_currency:
            if partner.country_id:
                # filter by country
                applicable_pricelist = self.env["product.pricelist"].search(
                    [
                        ("currency_id", "=", res_currency.id),
                        ("country_group_ids.country_ids", "=", partner.country_id.id),
                    ],
                    limit=1,
                )
                if not applicable_pricelist:
                    applicable_pricelist = self.env["product.pricelist"].search(
                        [("currency_id", "=", res_currency.id)], limit=1
                    )
            else:
                applicable_pricelist = self.env["product.pricelist"].search(
                    [("currency_id", "=", res_currency.id)], limit=1
        ...
        if date_order:
            try:
                datetime_obj = datetime.datetime.strptime(
                    date_order.split(".")[0], "%Y-%m-%dT%H:%M:%S"
                )
            except Exception as e:
        ...
        for line in product_lines:
            product_search_domain = [
                "|",
                ("barcode", "=ilike", line.get("ean")),
                ("default_code", "=ilike", line.get("id")),
            ]
            product = product_product.search(product_search_domain, limit=1)
            if not product:
                raise Warning(
                    _(
                        "Product %s not available in Odoo (order %s)."
                        % ((line.get("id") or line.get("ean")), order.get("id"))
                    )
                )
            created_line = self.create_order_line_from_channable(
        ...
        if price_data.get("shipping", 0):
            product = self.env.ref(
                "channable_sync.product_product_delivery_channable",
            ...
            if product:
                ...
                raise Warning(
                    _(
                        "Shipping cost service not available in Odoo, and shipping cost exists for this order."
                    )
                )
        ...
        if sale_order.company_id and sale_order.company_id.channable_auto_confirm_order:
            ...
            if (
                ...
                and sale_order.company_id.channable_auto_register_payment
            ):
                # and also register the payment on the invoice if applicable
                sale_order.write({"channel_process_payment": True})
            sale_order.action_confirm()

        msg = "Channable order import: successfully imported a new order: %s" % str(
            sale_order.name
        )
        _logger.info(msg)
        return msg
    ...
    def get_channable_customer(self, email, customer, billing, shipping, currency):
        ...
        partner = ResPartner.search(
            [
                ("email", "=", email),
                ("type", "not in", ["invoice", "delivery", "other"]),
                ("parent_id", "=", False),
            ],
            limit=1,
        )
        ...
        if partner:
            ...
            partner = self.update_partner_address(
                partner, customer, billing, is_company
            )

            partner_invoice = ResPartner.search(
                [("parent_id", "=", partner.id), ("type", "=", "invoice")], limit=1
            )
            partner_invoice = self.update_partner_address(
                partner_invoice, billing, customer, False
            )
            partner_shipping = ResPartner.search(
                [("parent_id", "=", partner.id), ("type", "=", "delivery")], limit=1
            )
            partner_shipping = self.update_partner_address(
                partner_shipping, shipping, customer, False
            )

            if not partner_invoice:
                partner_invoice = self.create_partner_address(
                    data=billing,
                    secondary_data=customer,
                    address_type="invoice",
                    is_company=False,
                    parent_id=partner and partner.id or False,
                )
            if not partner_shipping:
                partner_shipping = self.create_partner_address(
                    data=shipping,
                    secondary_data=customer,
            ...
            is_company = False if not customer.get("company") else True
            partner = self.create_partner_address(
                data=customer,
                secondary_data=billing,
                address_type=False,
    ...
    def create_partner_address(
        self, data, secondary_data, address_type, is_company, parent_id
    ):
        country_code = data.get("country_code", "") or secondary_data.get(
            "country_code", ""
        )
        country = self.env["res.country"].search([("code", "=", country_code)], limit=1)
        address2 = data.get("address2", "") or secondary_data.get("address2", "")
        address_supplement = data.get("address_supplement", "") or secondary_data.get(
            "address_supplement", ""
        )
        street2 = "{address2} {address_supplement}".format(
            address2=address2, address_supplement=address_supplement
        )
        names = [
            data.get("first_name", ""),
        ...
        return (
            self.env["res.partner"].with_context(skip_child_check=True).create(values)
        )
    ...
    def update_partner_address(self, partner, data, secondary_data, is_company=False):
        ...
        if is_company:
            ...
            name = "{first_name} {middle_name} {last_name}".format(
        ...
        if (
            ...
            or partner.city != data.get("city")
            or partner.street != address1
            or partner.street2 != street2
            or partner.country_id != country
            or partner.phone != phone
            or partner.mobile != mobile
            or partner.name != name
        ):
            if create_new:
                new_partner = self.create_partner_address(
                    data=data,
                ...
                partner.write(
                    {
                        ...
                        or (partner.country_id or partner.country_id.id),
                    }
                )
        return partner

    def create_order_line_from_channable(
        self, sale_order, line_data, product, is_delivery
    ):
        ...
        values = SaleOrderLine._convert_to_write(
            {name: temporary[name] for name in temporary._cache}
        )
        values.update(
            {
                "order_id": sale_order.id,
        ...
        if (
            ...
            and sale_order.company_id.channable_prices_contain_tax
        ):
    ...
    def update_status_in_channable(self):
        ...
        update_log = ""
        for order_to_cancel in self.search(
            [
                ("channable_order_id", "!=", False),
                ("channable_to_update_cancel", "=", True),
            ]
        ):
            order_to_cancel.update_status_canceled_channable()
            updated_orders_count += 1
            update_log += "Canceled order: %s\n" % order_to_cancel.name
        for order_to_update in self.search(
            [
                ("channable_order_id", "!=", False),
                ("channable_to_update_shipped", "=", True),
            ]
        ):
            order_to_update.update_status_shipped_channable()
            update_log += "Updated order: %s\n" % order_to_update.name
            updated_orders_count += 1

        msg = "Channable: updated status for %s orders.\n%s" % (
            str(updated_orders_count),
            update_log,
        )
        _logger.info(msg)
        return msg
    ...
    def update_status_canceled_channable(self):
        ...
        order_response = self.env.user.company_id.channable_request(
            method="GET",
        ...
        if order_response.get("order", {}).get("status_shipped", False) == "cancelled":
            ...
            self.env.user.company_id.channable_request(
                method="POST",
    ...
    def update_status_shipped_channable(self):
        ...
        if (
            order_response.get("order", {}).get("status_shipped", False)
            != "not_shipped"
        ):
            self.write(
                {
            ...
            response = self.env.user.company_id.channable_request(
                method="POST", endpoint=endpoint, payload=payload, timeout=120
            )
    ...
    def action_cancel(self):
        ...
        self.filtered(lambda o: o.channable_order_id).write(
            {"channable_to_update_cancel": True}
        )
        return res
    ...
    def action_view_in_channable(self):
        if self.channable_order_id:
            if (
                self.company_id
                and self.company_id.channable_company_id
                and self.company_id.channable_project_id
            ):
                url = "https://app.channable.com/companies/{company_id}/projects/{project_id}/orders/{order_id}".format(
                    company_id=self.company_id.channable_company_id,
    ...
    def process_channel_payment(self):
        for record in self.search(
            ...
            journal_id = self.env["account.journal"].search(
                [("channable_channel_name", "ilike", record.channable_payment_method)],
                limit=1,
            )
            if not journal_id:
                continue
    ...
    def _get_sale_order_has_issues(self):
        ...
        orders = self.search(
            [("channable_order_id", "!=", False), ("state", "=", "sale")]
        ).filtered(
        ...
        not_shipped_orders = self.search(
            [
                ("channable_order_id", "!=", False),
