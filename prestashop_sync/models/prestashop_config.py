...
class PrestashopConfig(models.Model):
    ...
    source_id = fields.Many2one(
        string="Source", comodel_name="utm.source", ondelete="restrict"
    )
    price_def_allowed = fields.Float(
        string="Allowed price difference", default=0.02, digits="Product Price"
    )
    ...
    def cron_get_orders(self):
        ...
        for Config in self.search([]):
            ...
            if CeleryTask.search_count(domain + [("ref", "=", get_order_ref)]) == 0:
                CeleryTask.call_task(
                    self._name,
                    "get_orders",
                    config=Config.id,
                    celery=get_orders_celery,
                    celery_task_vals={"ref": get_order_ref},
                )
        return True
    ...
    def get_orders(self, task_uuid, config, **kwargs):
        ...
        if self.batch_size:
            ...
            orders = Request.get(
                ...
                limit="%d,%d" % (offset, self.batch_size),
            ).get("orders", {})
            while orders:
        ...
        for order in all_orders:
            ...
            all_address_ids.extend(
                [
                    order.get("id_address_delivery", "0"),
                    order.get("id_address_invoice", "0"),
                ]
            )
        ...
        order_details = Request.get("order_details", params=order_details_params).get(
            "order_details", {}
        )
        order_rows = {}
        if order_details:
        ...
        all_customer_data = Request.get_by_ids(
            "customers",
            ids=all_customer_ids,
            fields="[id,id_lang,lastname,firstname,email,company]",
        )
        all_address_data = Request.get_by_ids("addresses", ids=all_address_ids)
        all_country_data = Request.get_by_ids(
            "countries", ids="all", fields="[id,iso_code]"
        )
        all_states_data = Request.get_by_ids(
            "states", ids="all", fields="[id,iso_code]"
        )
        updated = 0
        created = 0
        skipped = 0
            ...
            try:
                ...
                if sale_order:
                    if current_state in ("6", "146"):
                        CeleryTask.call_task(
                            "sale.order",
                            "update_from_prestashop",
                            so_id=sale_order.id,
                            order=order,
                            celery=celery,
                            celery_task_vals={
                                "ref": "Update existing order from prestashop: %s"
                                % sale_order.name
                            },
                        )
                    updated += 1
                # skip states canceled,payment process started, awaiting payment (adyen)
                    ...
                    order.get("id_carrier", "0")
                    invoice_data = all_address_data.get(invoice, {})
                    delivery_data = all_address_data.get(delivery, {})
                    delivery_country_code = all_country_data.get(
                        delivery_data.get("id_country", "0"), {}
                    ).get("iso_code", "")
                    invoice_country_code = all_country_data.get(
                        invoice_data.get("id_country", "0"), {}
                    ).get("iso_code", "")
                    delivery_state_code = all_states_data.get(
                        delivery_data.get("id_state", "0"), {}
                    ).get("iso_code", "")
                    invoice_state_code = all_states_data.get(
                        invoice_data.get("id_state", "0"), {}
                    ).get("iso_code", "")
                    if delivery_data:
                        delivery_data.update(
                            country_code=delivery_country_code,
                            state_code=delivery_state_code,
                        )
                    invoice_data.update(
                        country_code=invoice_country_code, state_code=invoice_state_code
                    )
                    data = {
                        "order": order,
                        "customer": customer,
                    ...
                    CeleryTask.call_task(
                        "sale.order",
                        "create_from_prestashop",
                        order=data,
                        celery=celery,
                        celery_task_vals={
                            "ref": "Create order from prestashop id: %s email: %s"
        ...
        return (
            "Pulled from API: %d\nUpdated orders: %d\nCreated orders: %d\nSkipped orders: %d"
            % (len(all_orders), updated, created, skipped)
        )

    def cron_sync_order_status(self):
        CeleryTask = self.env["celery.task"]
        domain = [("model", "=", self._name), ("state", "=", CELERY_STATE_PENDING)]
        sync_orders_celery = {
            "countdown": 1,
            "retry": True,
            "max_retries": 3,
        ...
        for Config in self.search([]):
            sync_order_ref = (
                "Sync Order status From Prestashop config_id: %d" % Config.id
            )
            mark_shipped_ref = (
                "Mark Order shipped to Prestashop config_id: %d" % Config.id
            )
            if (
                CeleryTask.search_count(
                    domain
                    + [
                        ("ref", "=", sync_order_ref),
                        ("method", "=", "sync_order_status"),
                    ]
                )
                == 0
            ):
                CeleryTask.call_task(
                    ...
                    "sync_order_status",
                    config=Config.id,
                    celery=sync_orders_celery,
                    celery_task_vals={"ref": sync_order_ref},
                )
            if (
                CeleryTask.search_count(
                    domain
                    + [
                        ("ref", "=", mark_shipped_ref),
                        ("method", "=", "prestashop_mark_shipped"),
                    ]
                )
                == 0
            ):
                CeleryTask.call_task(
                    self._name,
                    "prestashop_mark_shipped",
                    config=Config.id,
                    celery=sync_orders_celery,
                    celery_task_vals={"ref": mark_shipped_ref},
                )
        return True
    ...
    def sync_order_status(self, task_uuid, config, **kwargs):
        ...
        order_histories = Request.get("order_histories", params=params).get(
            "order_histories", {}
        )
        if order_histories:
        ...
        confirmed_prestashop_order_ids = [
            oh["id_order"] for oh in confirmed_prestashop_orders
        ]
        order_data_dict = Request.get_by_ids(
            "orders", ids=prestashop_order_ids, fields="[id,module,total_paid_tax_incl]"
        )
        for order in orders:
            data = order_data_dict[order.prestashop_id]
            if (
                order.prestashop_id in confirmed_prestashop_order_ids
            ):  # or (data['module'] == 'ps_wirepayment' and not order.invoice_ids):
                CeleryTask.call_task(
                    "sale.order",
                    "prestashop_order_process",
                    so_id=order.id,
                    data=data,
                    celery=celery,
                    celery_task_vals={"ref": order.name},
                )
        return True
    ...
    def prestashop_mark_shipped(self, task_uuid, config, **kwargs):
        ...
        if sync_ship_params:
            response = Request.request.add(
                "order_histories", content={"order_history": sync_ship_params}
            )
            if response:
                response_orders = response.get("prestashop", {}).get(
                    "order_history", []
                )
            ...
            SuccessOrders = ShippedOrders.filtered(
                lambda o: o.prestashop_id in response_order_ids
            )
