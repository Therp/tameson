...
class ResPartner(models.Model):
    ...
    def match_or_create_prestashop(self, order):
        ...
        if invoice_data:
            invoice, childs = childs.address_to_partner_prestashop(invoice_data, 'invoice', partner)
        ...
        if not delivery_data:
            ...
            delivery, childs = childs.address_to_partner_prestashop(delivery_data, 'delivery', partner)
        if childs:
            childs.write({'type': 'other'})
        if partner.vat and partner.country_id.country_group_ids.filtered(lambda g: g.name == 'Europe ex NL'):
            partner.property_account_position_id = self.env['account.fiscal.position'].search([('name','=','EU landen')])
        elif not partner.country_id.country_group_ids.filtered(lambda g: g.name == 'Europe'):
            partner.property_account_position_id = self.env['account.fiscal.position'].search([('name','=','Niet-EU landen')])
        else:
            pass
    ...
    def address_to_partner_prestashop(self, address, contact_type, parent):
        ...
        inv_country_id = self.env['res.country'].search([('code','=ilike',address.get('country_code', ''))], limit=1).id
        inv_state_id = self.env['res.country.state'].search([('code','=ilike',address.get('state_code', '')),('country_id','=',inv_country_id)], limit=1).id
        inv_phone = address.get('phone_mobile', False)
        inv_name = "%s %s" %(address.get('firstname', False), address.get('lastname', False))

        partner = self.filtered(lambda p: p.zip == inv_zip and \
                                    p.city == inv_city and \
                                    p.country_id.id == inv_country_id and \
                                    p.name == inv_name)[:1]
        if not partner:
            partner = self.create({
                'name': inv_name,
