...
class SaleOrderLine(models.Model):
    ...
    @api.depends(
        "product_uom_qty", "discount", "price_unit", "tax_id", "line_tax_amount_percent"
    )
                    ...
                    line.with_context(
                        {
                            "tax_computation_context": {
                                "line_tax_amount_percent": line_tax
                            }
                        }
                    ),
                )._compute_amount()
    ...
    def _prepare_invoice_line(self):
        ...
        if self.invoice_lines:
            res.update(
                {
                    "line_tax_amount_percent": self.invoice_lines[
                        0
