...
class AccountMoveLine(models.Model):
    ...
    line_tax_amount_percent = fields.Float(
        digits="Line Tax Amount", string="Tax amount in per(%)"
    )
    ...
    @api.onchange(
        "quantity", "discount", "price_unit", "tax_ids", "line_tax_amount_percent"
    )
    def _onchange_price_subtotal(self):
        ...
        for line in self:
            ...
            super(
                AccountMoveLine, line.with_context(context)
        ...
        if line_tax_details:
            return super(
                ...
                self.with_context(
                    {
                        "tax_computation_context": {
                            "line_tax_amount_percent": line_tax_details.get(
                                product.id, 0.0
                            )
                        }
                    }
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            line_tax_details.update(
                {vals.get("product_id"): vals.get("line_tax_amount_percent") or 0.0}
            )
