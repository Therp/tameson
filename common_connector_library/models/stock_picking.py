...
class StockPicking(models.Model):
    ...
    def get_set_product(self, product):
        try:
            ...
            bom_point = bom_obj.sudo()._bom_find(
                product=product, company_id=self.company_id.id, bom_type="phantom"
            )
            from_uom = product.uom_id
            to_uom = bom_point.product_uom_id
            factor = from_uom._compute_quantity(1, to_uom) / bom_point.product_qty
            bom, lines = bom_point.explode(
                product, factor, picking_type=bom_point.picking_type_id
            )
            return lines
        except:
            return {}
            ...
            if not is_kit_product:
                ...
                for move in move_lines:
                    ...
                    for operation in operations:
                        ...
                        qty_left = float_round(
                            float(qty_left) - op_qty,
                            precision_rounding=operation.product_uom_id.rounding,
                            rounding_method="UP",
                        )
                ...
                one_set_product_dict = self.get_set_product(
                    move_lines and move_lines[0], product
                )
                ...
                for product_id, bom_qty in transfer_product_qty.items():
                    ...
                    if float(qty_left) > 0.0 and float(op_qty):
                        stock_move_line_obj.create(
                            {
                                "product_id": product_move_lines
                                and product_move_lines[0].product_id.id,
                                "product_uom_id": product_move_lines
                                and product_move_lines[0].product_id.uom_id.id,
                                "picking_id": picking_id,
                                "qty_done": float(qty_left) or 0,
                                "result_package_id": package and package.id or False,
                                "location_id": picking_obj.location_id.id,
                                "location_dest_id": picking_obj.location_dest_id.id,
                                "move_id": product_move_lines
                                and product_move_lines[0].id,
                            }
                        )
                pick_ids.append(picking_id)
        if pick_ids:
            pickings = self.search(
                [("state", "=", "assigned"), ("id", "in", list(set(pick_ids)))]
            )
            pickings and pickings.action_done()
            pickings = self.search(
                [("state", "!=", "done"), ("id", "in", list(set(pick_ids)))]
            )
            pickings and pickings.action_done()
    ...
    def process_delivery_order_ept(
        self, picking_id, datas, traking_no=False, allow_extra_move=False
    ):
        move_obj = self.env["stock.move"]
        stock_move_line_obj = self.env["stock.move.line"]
        pick_ids = []
                    ...
                    stock_move_line_obj.create(
                        {
                            ...
                            "product_uom_id": move_lines
                            and move_lines[0].product_id.uom_id.id,
                            "picking_id": picking_id,
                            "qty_done": float(qty_left) or 0,
                            "location_id": picking_obj.location_id.id,
                            "location_dest_id": picking_obj.location_dest_id.id,
        ...
        move.copy(
            {
                ...
                "location_dest_id": datas.get("location_dest_id")
    ...
    def get_traking_number_for_phantom_type_product(
        self, picking, order_line_field=False
    ):
        line_items = {}
        update_move_ids = []
        move_obj = self.env["stock.move"]
        ...
        for move in move_lines:
            if move.sale_line_id.product_id.id != move.product_id.id:
                if (
                    move.sale_line_id in phantom_product_dict
                    and move.product_id.id
                    not in phantom_product_dict.get(move.sale_line_id)
                ):
                    phantom_product_dict.get(move.sale_line_id).append(
                        move.product_id.id
                    )
                else:
                    phantom_product_dict.update(
                        {move.sale_line_id: [move.product_id.id]}
                    )
        for sale_line_id, product_ids in phantom_product_dict.items():
            ...
            line_id = sale_line_id.search_read(
                [("id", "=", sale_line_id.id)], [order_line_field]
                ...
                if not tracking_no:
                    for move_line in move.move_line_ids:
                        tracking_no = (
                            ...
                            and move_line.result_package_id.tracking_no
                            or False
                        )
    ...
    def get_tracking_numbers(self, picking, order_line_field=False):
        ...
        line_items, update_move_ids = self.get_traking_number_for_phantom_type_product(
            picking, order_line_field
        )
        stock_moves = self.env["stock.move"].search(
            [("id", "not in", update_move_ids), ("picking_id", "=", picking)]
        )
        for move in stock_moves:
            line_id = move.sale_line_id.search_read(
                [("id", "=", move.sale_line_id.id)], [order_line_field]
            )[0].get(order_line_field)
