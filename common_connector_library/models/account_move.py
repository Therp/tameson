...
class AccountMove(models.Model):
    ...
    def create(self, vals):
        ...
        if (
            vals.get("line_ids")
            and vals.get("line_ids")[0][2]
            and vals.get("line_ids")[0][2].get("global_channel_id")
        ):
            vals.update(
                {
                    "global_channel_id": vals.get("line_ids")[0][2].get(
                        "global_channel_id"
                    )
                }
            )
    ...
    def _recompute_tax_lines(
        ...
        if not self.line_ids.filtered(
            lambda l: l.mapped("tax_ids").filtered(lambda l: l.amount_type == "code")
        ):
        ...
        def _compute_base_line_taxes(base_line):
            ...
            balance_taxes_res = base_line.tax_ids._origin.with_context(
                tax_computation_context={
                    "line_tax_amount_percent": base_line.line_tax_amount_percent
                },
        ...
        for taxes_map_entry in taxes_map.values():
            ...
            currency = (
                self.env["res.currency"].browse(currency_id) if currency_id else False
            )
            conversion_date = self.date or fields.Date.context_today(self)

            # Don't create tax lines with zero balance.
            if (currency or self.company_currency_id).is_zero(
