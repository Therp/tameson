...
class ProductProduct(models.Model):
    ...
    ept_image_ids = fields.One2many(
        "common.product.image.ept", "product_id", string="Images"
    )
    vendor_ids = fields.One2many(
        "vendor.stock.ept", "vendor_product_id", string="Vendor"
    )
    is_drop_ship_product = fields.Boolean(
        string="Is drop ship product",
        store=False,
        ...
        stock_rule = (
            ...
            .search(
                [
                    ...
                    ("action", "=", "buy"),
                    ("location_id", "in", customer_obj.ids),
    ...
    def get_products_based_on_movement_date(self, from_datetime, company=False):
        ...
        qry = """select product_id from stock_move where write_date >= '%s' and
                     state in ('partially_available','assigned','done', 'cancel','confirmed')""" % (
            date
        )
        if company:
    ...
    def get_vendor_stock_ept(self):
        ...
        category_obj = self.env["product.category"].search(
            [("route_ids", "in", stock_route_ids)]
        )
        product_obj = self.env["product.product"].search(
            [
                "|",
                ("route_ids", "in", stock_route_ids),
                ("categ_id", "in", category_obj.ids),
            ]
        )
        if product_obj:
            product_list_ids = ",".join(str(e) for e in product_obj.ids)
            qry = (
                """select vendor_product_id as product_id,
                    sum(vendor_stock) as stock from vendor_stock_ept
                    where vendor_product_id in (%s) group by vendor_product_id"""
                % product_list_ids
            )
            self._cr.execute(qry)
    ...
    def get_bom_product_stock_ept(
        ...
        mrp_module = module_obj.search(
            [("name", "=", "mrp"), ("state", "=", "installed")]
        )
        if not mrp_module:
            raise Warning("MRP module must be installed to do this process.")
        actual_stock = product_id.find_bom_product_possible_quantity(
            warehouse_id, stock_type
        )
        if actual_stock >= 1.00:
            if fix_stock_type == "fix":
                if fix_stock_value >= actual_stock:
    ...
    def find_bom_product_possible_quantity(
        self, warehouse_id, stock_type="virtual_available"
    ):
        """
        Added by Udit
        ...
        for record in bom_lines:
            ...
            product = self.with_context(warehouse=warehouse_id).browse(
                record[0].product_id.id
            )
            actual_stock = getattr(product, stock_type)
            possible_combination = (
                int(actual_stock / bom_product_qty)
                if actual_stock > 0 and bom_product_qty > 0
                else 0
            )
            if flag:
