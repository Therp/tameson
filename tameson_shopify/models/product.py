...
mutation {
    bulkOperationRunQuery(
        ...
        session = shopify.Session(
            instance.shopify_host, "2021-04", instance.shopify_password
        )
        shopify.ShopifyResource.activate_session(session)
        json.loads(shopify.GraphQL().execute(query_all_sku_available))
    ...
    def compare_and_sync(self, instance_id, gid):
        ...
        levels = tuple(
            self.env["shopify.stock.level"].create(process_export(lines)).ids
        )
...
where sp.id is null and sl.id in %s""" % (
            instance.id,
            str(levels),
        )
        ...
        chunked_map = [
            missing_map_data[i : i + 10000]
            for i in range(0, len(missing_map_data), 10000)
        ]
...
and sl.id in %s""" % (
        ...
        chunked_mismatch = [
            mismatch_products[i : i + 100]
            for i in range(0, len(mismatch_products), 100)
        ]
    ...
    def update_stock_all_shop(self):
        ...
        to_update_products = (
            self.env["stock.move.line"].search(domain).mapped("product_id")
            + self.env["stock.move"].search(domain).mapped("product_id").ids
        )
        bom_product_query = """
...
WHERE bl.product_id IN (%s)""" % ",".join(
            map(str, to_update_products)
        )
        ...
        chunked_mismatch = [
            to_update[i : i + 100] for i in range(0, len(to_update), 100)
        ]
    ...
    def sync_mismatch_qty(self, mismatch_products, instance):
        ...
        if mismatch_products:
            ...
            shopify_products = shopify_product_obj.with_context(
                is_process_from_selected_product=True
            ).export_stock_in_shopify(instance, product_id_array)
        return True
...
class ShopifyProductProductEpt(models.Model):
    ...
    def check_stock_type(self, instance, product_ids, prod_obj, warehouse):
        ...
        for i in product_data:
            product_stock_dict.update(
                {i.get("id"): i.get("minimal_qty_available_stored")}
            )
