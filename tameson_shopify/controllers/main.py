...
class Shopify(Controller):
    ...
    @route(
        ["/shopify/export_done/<int:instance_id>"],
        type="json",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    def shopify_export_done(self, instance_id, **kw):
        ...
        request.httprequest.headers["X-Shopify-Shop-Domain"]
        request.env[
            "shopify.process.import.export"
        ].sudo().with_delay().compare_and_sync(instance_id, bulk)
    ...
    @route(
        [
            "/shopify/customer_create/<int:instance>",
            "/shopify/customer_update/<int:instance>",
        ],
        type="json",
        auth="public",
        methods=["POST"],
                ...
                .search(
                    [
                        (
                            "name",
                            "=",
                            "shopify.instance.ept.process_customer_webhook_data",
                        ),
                        ("func_string", "ilike", email),
