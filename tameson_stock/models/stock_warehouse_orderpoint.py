...
class StockWarehouseOrderpoint(models.Model):
    ...
    rr_supplier = fields.Char(
        string=_("Supplier"), compute="_compute_rr_supplier", store=True
    )
    rr_out_3m = fields.Integer(
        string=_("Out 3M"), compute="_compute_rr_out_3m", store=True
    )
    rr_out_12m = fields.Integer(
        string=_("Out 12M"), compute="_compute_rr_out_12m", store=True
    )
    rr_max_out_3m = fields.Float(
        string=_("Max/out 3M"), compute="_compute_rr_max_out_3m", store=True
    )
    rr_count_3m = fields.Integer(
        string=_("Count 3M"), compute="_compute_rr_count_3m", store=True
    )
    rr_count_12m = fields.Integer(
        string=_("Count 12M"), compute="_compute_rr_count_12m", store=True
    )
    ...
    def cron_validate_orderpoint_product_routes(self):
        ...
        orderpoints_to_remove = self.env["stock.warehouse.orderpoint"].search(
            [("product_max_qty", "<=", 0.0)]
        )
    ...
    def _compute_rr_supplier(self):
        ...
        suppliers = {row[0]: row[1] for row in self.env.cr.fetchall()}
    ...
    def _rr_count_pickings(self, picking_type_code, months):
        move_lines_dict = self._rr_move_lines_dict(
            picking_type_code, months, operator="COUNT(*)"
        )

        return {
            record.id: move_lines_dict.get(record.product_id.id, 0) for record in self
        }

    def _rr_sum_qtys(self, picking_type_code, months):
        move_lines_dict = self._rr_move_lines_dict(
            picking_type_code, months, operator="SUM(sm.product_qty)"
        )
    ...
    def _rr_move_lines_dict(self, picking_type_code, months, operator="COUNT(*)"):
        ...
        """.format(
            months=months, operator=operator
        )
