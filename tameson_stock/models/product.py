...
class ProductProduct(models.Model):
    ...
    def _get_min_components_needs(self, bom):
        ...
        for bom_line, bom_component in bom_components:
            ...
            #     component.uom_id
            # )
            needs += Counter({component: bom_component["qty"]})
        return needs
    ...
    def _minimal_qty_available(self, field_names=None, arg=False):
        ...
        for product in self:
            ...
            if product.bom_ids:
                ...
                components = bom_products.mapped("bom_ids.bom_line_ids.product_id")
                non_bom_products._minimal_qty_available()
                components._minimal_qty_available()
                for product in bom_products:
                    bom_qtys = {0.0}
                    for bom in product.bom_ids:
                        component_needs = product._get_min_components_needs(bom)
            ...
            product.minimal_qty_available = float_round(
                min_qty, precision_rounding=product.uom_id.rounding
            )
...
class ProductTemplate(models.Model):
    ...
    def _get_buy_route(self):
        mtos_id = self.env.ref(
            "stock_mts_mto_rule.route_mto_mts", raise_if_not_found=False
        )
    ...
    def write(self, vals):
        ...
        if warehouse_id:
            ...
            rules.write({"warehouse_id": warehouse_id})
    ...
    def _compute_has_reordering_rules(self):
        ...
        for product in self:
            product.rr_has_reordering_rules = have_rules.get(
                product.product_variant_id.id, False
            )
    ...
    def cron_recompute_all_min_qty_avail_stored_tmpl(self, split=15000):
        to_update_products = self.env["stock.move"].search([]).mapped("product_id")
...
WHERE pp.id IN (%s)""" % ",".join(
            map(str, self.mapped("product_variant_ids").ids)
        )
        ...
        for pos in range(0, len(to_update_product_tmpls), split_size):
            to_update_product_tmpls[
                pos : pos + split_size
            ].with_delay().store_min_qty_jobs()
