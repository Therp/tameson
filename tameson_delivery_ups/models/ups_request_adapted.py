...
NULL_PHONE_RE = re.compile('^[0\s]$')  # Validation for empty phone number
...
class UPSRequest(object):
    ...
    def sanitize_addresses(address_lines):
        ...
        for line in address_lines:
            if line:
                ret.extend(UPSRequest.split_by_length(line, UPSRequest.MAX_ADDRESS_LENGTH))
    ...
    def get_error_message(self, error_code, description):
        result = {}
        result['error_message'] = 'UPS ERROR %s (%s): %s' % (error_code, description, UPS_ERROR_MAP.get(error_code))
    ...
    def get_shipping_price(self, shipment_info, packages, shipper, ship_from, ship_to, packaging_type, service_type, saturday_delivery, cod_info):
        ...
        shipment.Shipper.Address.AddressLine = self.sanitize_addresses([shipper.street, shipper.street2])
        ...
        shipment.ShipFrom.Address.AddressLine = self.sanitize_addresses([ship_from.street, ship_from.street2])
        ...
        shipment.ShipTo.Address.AddressLine = self.sanitize_addresses([ship_to.street, ship_to.street2])
    ...
    def send_shipping(self,
        ...
        shipment.Shipper.Name = (shipper.parent_id.name or shipper.name or '')[:self.NAME_LENGTH]
        shipment.Shipper.Address.AddressLine = self.sanitize_addresses([shipper.street, shipper.street2])
        shipment.Shipper.Address.City = shipper.city or ''
        shipment.Shipper.Address.PostalCode = shipper.zip or ''
        shipment.Shipper.Address.CountryCode = shipper.country_id.code or ''
        ...
        shipment.ShipFrom.Name = (ship_from.parent_id.name or ship_from.name or '')[:self.NAME_LENGTH]
        shipment.ShipFrom.Address.AddressLine = self.sanitize_addresses([ship_from.street, ship_from.street2])
        shipment.ShipFrom.Address.City = ship_from.city or ''
        shipment.ShipFrom.Address.PostalCode = ship_from.zip or ''
        shipment.ShipFrom.Address.CountryCode = ship_from.country_id.code or ''
        ...
        shipment.ShipTo.Name = (ship_to.parent_id.name or ship_to.name or '')[:self.NAME_LENGTH]
        shipment.ShipTo.Address.AddressLine = self.sanitize_addresses([ship_to.street, ship_to.street2])
        shipment.ShipTo.Address.City = ship_to.city or ''
        shipment.ShipTo.Address.PostalCode = ship_to.zip or ''
        shipment.ShipTo.Address.CountryCode = ship_to.country_id.code or ''
    ...
    def process_shipment(self):
        ...
        try:
            ...
            if response.Response.ResponseStatus.Code != "1":
                ret = self.get_error_message(response.Response.ResponseStatus.Code, response.Response.ResponseStatus.Description)
                ret['error_message'] = '%s\n\n%s\n\n%s\n\n%s' % (ret['error_message'], self.request, self.shipment, self.label)
                return ret
            ...
            ret['error_message'] = '%s\n\n%s\n\n%s\n\n%s\n\n%s' % (ret['error_message'], self.request, self.shipment, self.label, str(e))
            return ret
        except IOError as e:
            return self.get_error_message('0', 'UPS Server Not Found:\n%s' % e)
    ...
    def _shipment_notification(
        ...
        ret = {
            'ShipmentServiceOptions': self.factory_ns2.ShipmentServiceOptionsType()
        }
        ...
        if not access_point_id and access_point_country:
            raise Exception('UPS Access Point country given but access point ID not known')
        ...
        if access_point_id:
            ...
            access_point_res = locator.get_access_point(access_point_id, access_point_country)
            ...
            if not send_notification_emails:
                raise Exception('Send notification emails is empty but notification sending was requested')
...
class UPSLocator(object):
    ...
    def request_body(self, data=None):
            ...
            'LocatorRequest': {
                ...
                'LocationSearchCriteria': {
                    'SearchOption': {
                        ...
                        'OptionCode': {
                            'Code': self.OPTIONS['Location']['codes']['ups-access-point'],
                        },
                    },
                },
        ...
        if 'street' in data and \
               'postal-code' in data and \
               'country-code' in data:
            address_dict = {
