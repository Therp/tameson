...
from .ups_request_adapted import Package, UPSRequest


class ProviderUPS(models.Model):
    _inherit = "delivery.carrier"

    ups_default_package_weight = fields.Float(
        string="Default package weight",
        help="If any of products in package has undefined weight, this value will be used",
        default=1.0,
    )
    ups_send_notification_to_customer = fields.Boolean(
        string="UPS sends email notifications to the customer", default=False
    )
    ups_send_notification_email = fields.Char(
        string="UPS notification additional email"
    )
    ups_send_notification_undelivered_email = fields.Char(
        string="UPS email notification to this email when customer email not delivered"
    )
    ...
    def ups_rate_shipment(self, order):
        ...
        check_value = srm.check_required_value(
            ...
            order=order,
            default_package_weight=superself.ups_default_package_weight,
        )  # Tameson edit: add default weight
        if check_value:
            return {
                "success": False,
                "price": 0.0,
    ...
    def ups_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            if picking.package_ids:
                ...
                for (
                    package
                ) in picking.package_ids:  # Tameson edit: add default package weight
                    packages.append(
                        Package(
                            self,
                            package.shipping_weight
                            or superself.ups_default_package_weight,
            ...
            if picking.weight_bulk or (
                not packages and superself.ups_default_package_weight
            ):  # Tameson edit: add default package weight
                packages.append(
                    Package(
                        self,
                        picking.weight_bulk or superself.ups_default_package_weight,
                    )
                )
            ...
            if picking.sale_id and picking.sale_id.carrier_id != picking.carrier_id:
                ups_service_type = (
                    picking.carrier_id.ups_default_service_type
                    or picking.ups_service_type
                    or self.ups_default_service_type
                )
            else:
                ups_service_type = (
                    picking.ups_service_type or self.ups_default_service_type
                )
            ...
            package_type = (
                ...
                or self.ups_default_packaging_id.shipper_package_code
            )
            ...
            srm.send_shipping(
                ...
                send_notification_emails=[
                    partner_email,
                    superself.ups_send_notification_email,
                ],
            ...
            if result.get("error_message"):
                ...
                error = "Orders: %s\n\n%s" % (
                    ", ".join(
                        "%s [picking %s]" % (sp.origin, sp.name) for sp in pickings
                    ),
            ...
            logmessage = _(
                ...
                "<b>Packages:</b> %s"
            ) % (carrier_tracking_ref, ",".join(package_names))
    ...
    def ups_get_return_label(self, picking, tracking_number=None, origin_date=None):
        ...
        if picking.is_return_picking:
            weight = sum(
                m.product_id.weight * m.product_qty for m in picking.move_lines
            )
        ...
        if self.ups_label_file_type == "GIF":
            attachments = [
                (
                    "%s-%s-%s.%s"
                    % (self.get_return_label_prefix(), package_labels[0][0], 1, "pdf"),
    ...
    def ups_cancel_shipment(self, picking):
        ...
        if result.get("error_message"):
            ...
            picking.message_post(
                body=_(
                    "Shipment N° %s has been cancelled" % picking.carrier_tracking_ref
                )
            )
