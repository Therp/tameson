...
class SFTPServer(models.Model):
    ...
    auth_type = fields.Selection(AUTH_TYPES, string='Authentification Type', required=True)
            ...
            try:
                ...
                if sftp_server.protocol == 'ftp':
                    with ftplib.FTP() as ftp:
                        ...
                        ftp.voidcmd("NOOP")
                        _logger.info('FTP working: {server}'.format(server=sftp_server.name))
                else:
                    ...
                    with paramiko.Transport((sftp_server.host_address, sftp_server.port)) as sftp_transport:
                        ...
                        with paramiko.SFTPClient.from_transport(sftp_transport) as sftp:
                            ...
                            _logger.info('SFTP: Current directory: {curr_dir}'.format(curr_dir=sftp.listdir()))
