...
class CustomExporter(models.Model):
    ...
    custom_export_format_header = fields.Char(
        ...
        help="Modify the header columns if needed.",
        required=True,
    )
    ...
    cron_interval_number = fields.Integer(
        related="ir_cron_id.interval_number",
        string="Cron Interval Number",
        readonly=False,
    )
    cron_interval_type = fields.Selection(
        related="ir_cron_id.interval_type", string="Cron Interval Type", readonly=False
    )
    ...
    nextcall = fields.Datetime(
        string="Next Export At",
        required=True,
        default=fields.Datetime.now,
        help="Next planned execution date for this export.",
    )
    ...
    def unlink(self):
        for rec in self:
            custom_export_files = self.env["custom.export.file"].search(
                [("custom_exporter_id", "=", rec.id)]
            )
            if custom_export_files:
                raise ValidationError(
                    _(
                        "This exporter already produced files, so it can not be deleted - please archive it or instead delete all the exported files first if you want to delete it."
                    )
                )
            else:
        ...
        for custom_exporter in self:
            if not custom_exporter.ir_cron_id:
                ...
                vals = {
                    ...
                    "code": "model._cron_run_custom_export({custom_exporter_id})".format(
                        custom_exporter_id=custom_exporter.id
                    ),
                    "numbercall": -1,
    ...
    def update_custom_header(self):
        if self.custom_export_format_id:
            ...
            self.custom_export_format_header = [
                val["label"].strip() for val in fields_name
            ] or ""
    ...
    def _cron_run_custom_export(self, custom_exporter_id=None):
        ...
        try:
            ...
            msg = str(e)
            follower_ids = (
                new_custom_export_file.message_follower_ids.mapped("partner_id").ids
                or []
            )
            new_custom_export_file.message_post(
                body=_("Custom Exporter Error:\n\n %s.") % (msg,),
                message_type="email",
                partner_ids=follower_ids,
            )
            _logger.warning(msg)
        file_obj = custom_exporter.generate_custom_export(
            new_custom_export_file, recordset
        )
        new_custom_export_file.attach_and_export_file(file_obj, records_exported)
        if custom_exporter.retention_days > 0:
            date = datetime.date.today() - relativedelta.relativedelta(
                days=custom_exporter.retention_days
            )
    ...
    def get_custom_format_namelist(self, model, export_id):
        ...
        export_fields_list = (
            self.env["ir.exports.line"].browse(export["export_fields"]).read()
        )

        fields_data = fields_info(self, model, [f["name"] for f in export_fields_list])
    ...
    def generate_custom_export(self, new_custom_export_file, recordset):
        ...
        model_name = self.export_model_name
        try:
            fields_name = self.get_custom_format_namelist(
                model_name, self.custom_export_format_id.id
            )
            ids = recordset.ids
        except Exception as e:
        ...
        records = model.browse(ids) or model.search(
            self.export_domain or [], offset=0, limit=False, order=False
        )
    ...
    def create_custom_export_file(self):
        ...
        if self.fixed_filename:
            active_export = self.env["custom.export.file"].search(
                [("custom_exporter_id", "=", self.id), ("state", "=", "draft")], limit=1
            )
            if active_export:
                raise ValidationError(
                    _(
        ...
        if self.fixed_filename:
            ...
            filename = name
        vals = {"name": name, "filename": filename, "custom_exporter_id": self.id}
        ...
        if self.message_follower_ids:
            ...
            custom_export_file.message_subscribe(
                partner_ids=following_partners.ids, subtype_ids=subtype_ids
            )
    ...
    def export_via_sftp(self, file_obj):
        for custom_export_file in self:
            ...
            if self._context.get("skip_sftp_transfer_when_testing"):
                ...
                _logger.info(
                    "Testing: {src_file} file successfully exported.".format(
            ...
            try:
                ...
                if sftp_server.protocol == "ftp":
                    with ftplib.FTP() as ftp:
                        ...
                        _logger.info(
                            "FTP Exporting: {src_path}/{src_file}".format(
                                src_path=sftp_server.export_path,
                                src_file=custom_export_file.filename,
                            )
                        )
                        filelike_obj = io.BytesIO(base64.decodebytes(file_obj))
                        ftp.storbinary(
                            "STOR %s" % custom_export_file.filename, filelike_obj
                        )
                        _logger.info(
                            "FTP Exported File: {src_path}/{src_file}".format(
                                src_path=sftp_server.export_path,
                                src_file=custom_export_file.filename,
                            )
                        )
                    ...
                    with paramiko.Transport(
                        ...
                        with paramiko.SFTPClient.from_transport(sftp_transport) as sftp:
                            ...
                            _logger.info(
                                "SFTP Exporting: {src_path}/{src_file}".format(
                                    src_path=sftp_server.export_path,
                                    src_file=custom_export_file.filename,
                                )
                            )
                            ...
                            _logger.info(
                                "SFTP Exported File: {src_path}/{src_file}".format(
                                    src_path=sftp_server.export_path,
                                    src_file=custom_export_file.filename,
                                )
                            )
                ...
                msg = str(e)
                follower_ids = (
                    custom_export_file.message_follower_ids.mapped("partner_id").ids
                    or []
                )
                custom_export_file.message_post(
                    body=_("Custom Exporter FTP/SFTP Error:\n\n %s.") % (msg,),
                    message_type="email",
                    partner_ids=follower_ids,
                )
                _logger.warning(msg)
