...
class TestCustomExporter(SavepointCase):
    ...
    def setUp(self):
        ...
        self.custom_export_format = self.env['ir.exports'].create({
            ...
            'export_fields': [(0, 0, {'name': 'sequence'}),
    ...
    def test_call_custom_exporter_testing(self):
        # run the exporter
        self.env['custom.exporter'].with_context(skip_sftp_transfer_when_testing=True)._cron_run_custom_export(self.test_custom_exporter.id)
        # make sure there is 1 export file record
        export_files = self.env['custom.export.file'].search([('custom_exporter_id', '=', self.test_custom_exporter.id)])
