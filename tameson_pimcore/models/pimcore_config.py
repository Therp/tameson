...
product_nodes = {
    ...
    "draft": {"field": "Draft", "getter": static_getter},
}
...
class PimcoreConfig(models.Model):
    ...
    def action_fetch_products(self):
        ...
        try:
            record_count = (
                pim_request.execute(
                    "{getProductListing(published: false) {totalCount}}"
                )
    ...
    def action_fetch_new(self):
        ...
        pim_request = PimcoreRequest(self.api_host, self.api_name, self.api_key)
        filter = '\\"modificationDate\\" : {\\"$gt\\": \\"%.1f\\"}' % last_mdate
    ...
    def action_fetch_products_jobs(self):
        ...
        params = 'first: %d, sortBy: "id",  sortOrder: "%s"'
        try:
