...
class Main(http.Controller):
    ...
    def shopify_odoo_webhook_for_customer_update(self):
        res, instance, webhook = self.get_basic_info(
            "shopify_odoo_webhook_for_customer_update"
        )
        if res and instance.active and webhook.state == "active":
            ...
            if res.get("addresses"):
                process_import_export_model = request.env[
                    "shopify.process.import.export"
                ].sudo()
                process_import_export_model.webhook_customer_create_process(
                    res, instance
                )
                request._cr.commit()
        else:
            _logger.info(
                "%s instance is Archived or 'customers/update' webhook event is currently Inactive."
                "So functionality of the webhook is temporarily disable."
                % instance.name
            )
        return

    @http.route(
        "/shopify_odoo_webhook_for_order_create", csrf=False, auth="public", type="json"
    )
    ...
    @http.route(
        "/shopify_odoo_webhook_for_orders_partially_updated",
        csrf=False,
        auth="public",
        type="json",
    )
    def update_order_webhook(self):
        ...
        if not instance.active or not odoo_webhook.state == "active":
            _logger.info(
                "The method is skipped. It appears the instance:{} is not active or that the "
                "webhook{} is not active."
                "".format(instance.name, odoo_webhook.webhook_name)
            )
            return
        _logger.info("UPDATE ORDER WEBHOOK call for order: {}".format(res.get("name")))
        ...
        if (
            ...
            .search_read(
                [
                    ...
                    ("shopify_order_number", "=", res.get("order_number")),
                ],
                ["id"],
            )
        ):
            ...
            if res.get(
                "fulfillment_status"
            ) in instance.import_shopify_order_status_ids.mapped("status"):
                request.env["sale.order"].sudo().process_shopify_order_via_webhook(
                    res, instance
                )
        return
    ...
    @http.route(
        "/shopify_odoo_webhook_for_orders_partially_cancelled",
        csrf=False,
        auth="public",
        type="json",
    )
    def cancel_order_webhook(self):
        ...
        if not instance.active or not odoo_webhook.state == "active":
            ...
            return
        _logger.info("CANCEL ORDER WEBHOOK call for order: {}".format(res))
    ...
    @http.route(
        "/shopify_odoo_webhook_for_product", csrf=False, auth="public", type="json"
    )
    ...
    @http.route(
        "/shopify_odoo_webhook_for_product_update",
        csrf=False,
        auth="public",
        type="json",
    )
    ...
    @http.route(
        "/shopify_odoo_webhook_for_product_delete",
        csrf=False,
        auth="public",
        type="json",
    )
    def delete_product_webhook(self):
        ...
        if not instance.active or not webhook.state == "active":
            ...
            return
        _logger.info(
            "DELETE PRODUCT WEBHOOK call for product: {}".format(request.jsonrequest)
        )
    ...
    def product_webhook_process(self, route):
        ...
        if route == "shopify_odoo_webhook_for_product":
            _logger.info(
                "CREATE PRODUCT WEBHOOK call for product: {}".format(
                    request.jsonrequest.get("title")
                )
            )
        elif route == "shopify_odoo_webhook_for_product_update":
            _logger.info(
                "UPDATE PRODUCT WEBHOOK call for product: {}".format(
                    request.jsonrequest.get("title")
                )
            )

        woo_template = (
            request.env["shopify.product.template.ept"]
        ...
        if woo_template:
            ...
            ].sudo().create_product_queue_from_webhook(res, instance)

        elif res.get("published_at"):
            request.env[
                "shopify.product.data.queue.ept"
            ].sudo().create_product_queue_from_webhook(res, instance)
    ...
    def get_basic_info(self, route):
        ...
        instance = (
            ...
            .sudo()
            .with_context(active_test=False)
            .search([("shopify_host", "ilike", host)], limit=1)
        )
        ...
        webhook = (
            request.env["shopify.webhook.ept"]
            .sudo()
            .search(
