...
class ShopifyProductProductEpt(models.Model):
    ...
    shopify_instance_id = fields.Many2one(
        "shopify.instance.ept", "Instance", required=1
    )
    ...
    fix_stock_type = fields.Selection(
        [("fix", "Fix"), ("percentage", "Percentage")], string="Fix Stock Type"
    )
    ...
    shopify_image_ids = fields.One2many(
        "shopify.product.image.ept", "shopify_variant_id"
    )  # add by Bhavesh Jadav 17/12/209  for the variant images
    last_stock_update_date = fields.Datetime(readonly=True)
    ...
    def toggle_active(self):
        ...
        with_one_active = self.filtered(
            lambda product: len(product.shopify_template_id.shopify_product_ids) == 1
        )
    ...
    def shopify_search_product_varint_in_odoo(self, instance, barcode, sku):
        ...
        if (
            not shopify_variant
            and instance.shopify_sync_product_with == "sku_or_barcode"
        ):
            if not shopify_variant and barcode:
                shopify_variant = self.search(
                    [
    ...
    def shopify_create_variant_product(
        self,
        result,
        instance,
        price,
        ...
        if attrib_line_vals:
            ...
            available_odoo_products = self.shopify_set_variant_sku(
                result, product_template, instance, import_data_id
    ...
    def shopify_prepare_attribute_vals(self, result):
        ...
        for attrib in result.get("options"):
            ...
            attribute = product_attribute_obj.search(
            ...
            for attrib_vals in attrib_values:
                ...
                if not attrib_value:
                    attrib_value = product_attribute_value_obj.with_context(
                        active_id=False
                    ).create({"attribute_id": attribute.id, "name": attrib_vals})
    ...
    def shopify_update_price(self, instance, product_template, price):
        ...
        """
        if (
            instance.shopify_pricelist_id.currency_id.id
            ...
            amount = instance_currency._convert(
                float(price), product_company_currency, company, date
            )
            product_template.write({"list_price": amount})

        return True

    def shopify_set_variant_sku(
        self, result, product_template, instance, import_data_id
    ):
        ...
        product_attribute_obj = self.env["product.attribute"]
        product_attribute_value_obj = self.env["product.attribute.value"]
        product_template_attribute_value_obj = self.env[
        ...
        for variation in result.get("variants"):
            ...
            for variation_attribute in variation_attributes:
                ...
                if product_attribute_value:
                    template_attribute_value_id = (
                        product_template_attribute_value_obj.search(
                            [
                                (
                    ...
                    template_attribute_value_id and template_attribute_value_ids.append(
            ...
            odoo_product and available_odoo_products.update(
                {variation["id"]: odoo_product}
            )  # add by bhavesh jadav
            if barcode:
                existing_barcode_product = odoo_product_obj.search(
                    [("barcode", "=", barcode)], limit=1
                )
                if (
                    existing_barcode_product
                    and existing_barcode_product.id != odoo_product.id
                ):
            ...
            if price:
                if (
                    ...
                    odoo_product and odoo_product.write(
    ...
    def shopify_export_products(
        ...
        for template in templates:
            ...
            if is_set_basic_detail or is_publish:
                self.shopify_set_template_value_in_shopify_obj(
                    new_product, template, is_publish, is_set_basic_detail
                )
            if is_set_basic_detail or is_set_price:
                variants = []
                for variant in template.shopify_product_ids:
                    variant_vals = self.shopify_prepare_variant_vals(
                        instance, variant, is_set_price, is_set_basic_detail
                    )
            ...
            if not result:
                message = (
                    "Product %s Is Not Export In Shopify Store while Export Product"
                    % (template.name)
                )
                self.shopify_export_product_log_line(message, model_id, log_book_id)
            if result:
                self.update_products_details_Shopify_third_layer(
                    new_product, template, is_publish
                )
    ...
    def update_products_in_shopify(
        self,
        instance,
        is_set_price,
        is_set_images,
        ...
        if (
            not is_publish
            and not is_set_basic_detail
            and not is_set_price
            and not is_set_images
        ):
        ...
        for template in templates:
            ...
            if is_publish == "publish_product":
                ...
                template.write(
                    {"published_at": published_at, "website_published": True}
                )
    ...
    def shopify_set_template_value_in_shopify_obj(
        self, new_product, template, is_publish, is_set_basic_detail
    ):
        """
    ...
    def shopify_prepare_variant_vals(
        self, instance, variant, is_set_price, is_set_basic_detail
    ):
        ...
        if is_set_price:
            price = instance.shopify_pricelist_id.get_product_price(
                variant.product_id,
                1.0,
                partner=False,
            ...
            variant_vals.update(
                {
                    ...
                    "taxable": variant.shopify_template_id.taxable
                    and "true"
                    or "false",
                    "title": variant.name,
                }
            ...
            att_values = attribute_value_obj.search(
                [
                    (
                        "id",
                        "in",
                        variant.product_id.product_template_attribute_value_ids.ids,
                    )
                ],
                order="attribute_id",
            )
    ...
    def update_products_details_Shopify_third_layer(
        self, new_product, template, is_publish
    ):
        """
        ...
        if is_publish:
            template_vals.update(
                {"published_at": datetime.now(), "website_published": True}
            )
        ...
        for variant_dict in result_dict.get("variants"):
            ...
            shopify_variant = template.shopify_search_odoo_product_variant(
                template.shopify_instance_id,
    ...
    def create_image_record(
        self, instance, shopify_image, shopify_template, product_template, product
    ):
        product_image_object = self.env["common.product.image.ept"]
        vals = {
            "image": product_template.image_1920
            if product_template
            else product.image_1920,
        ...
        product_image_object.create(vals)

    def update_image_record(
        self, instance, shopify_image, shopify_template, image_record
    ):
        vals = {
            "shopify_product_tmpl_id": shopify_template.id,
            "shopify_image_id": shopify_image.id,
    ...
    def update_product_images(self, instance, shopify_template):
        ...
        try:
            shopify_images = shopify.Image().find(
                product_id=int(shopify_template.shopify_tmpl_id)
            )
        except Exception as e:
            if e.response.code == 429 and e.response.msg == "Too Many Requests":
                time.sleep(int(float(e.response.headers.get("Retry-After", 5))))
            ...
            if not image.shopify_image_id:
                ...
                if image.shopify_variant_id:
                    shopify_image.variant_ids = [
                        int(image.shopify_variant_id.variant_id)
                    ]
                result = shopify_image.save()
                if result:
                    image.write({"shopify_image_id": shopify_image.id})
                ...
                for shop_image in shopify_images:
                    if int(image.shopify_image_id) == shop_image.id:
                        ...
                        shopify_image.attachment = image.odoo_image_id.image.decode(
                            "utf-8"
                        )
                        result = shopify_image.save()
        return True
    ...
    def export_stock_in_shopify(self, instance=False, products=False):
        ...
        if self._context.get("is_process_from_selected_product"):
            ...
            if instance.shopify_last_date_update_stock:
                shopify_products = all_products.filtered(
                    lambda x: not x.last_stock_update_date
                    or x.last_stock_update_date
                    <= instance.shopify_last_date_update_stock
                )
            else:
                shopify_products = all_products.filtered(
                    lambda x: not x.last_stock_update_date
        ...
        if not location_ids:
            message = "Location not found for instance %s while update stock" % (
                instance.name
            )
            log_line_array = self.shopify_create_log(
        ...
        for location_id in location_ids:
            ...
            if not shopify_location_warehouse:
                message = (
                    "No Warehouse found for Export Stock in Shopify Location: %s"
                    % (location_id.name)
                )
                log_line_array = self.shopify_create_log(
                    message, model_id, False, log_line_array
            ...
            for shopify_product in shopify_products:
                ...
                if odoo_product.type == "product":
                    if not shopify_product.inventory_item_id:
                        message = (
                            "Inventory Item Id did not found for Shopify Poduct Vatiant ID "
                            "%s with name %s for instance %s while Export stock"
                            % (shopify_product.id, shopify_product.name, instance.name)
                        )
                        log_line_array = self.shopify_create_log(
                            message, model_id, odoo_product, log_line_array
                        )
                        continue
                    quantity = export_product_stock.get(odoo_product.id, 0.0)
                    try:
                        _logger.info(
                            "Exporting product stock: %s of shopify product default code:%s"
                            % (str(quantity), shopify_product.default_code)
                        )
                        ...
                        if (
                            ...
                            time.sleep(
                                int(float(e.response.headers.get("Retry-After", 5)))
                            )
                            try:
                                shopify.InventoryLevel.set(
                                    location_id.shopify_location_id,
                                ...
                                message = (
                                    "Error while Export stock for Product ID: %s & Product Name: '%s' for instance: '%s'\nError: %s"
                                    % (
                                        odoo_product.id,
                                        odoo_product.name,
                                        instance.name,
                    ...
                    if not self._context.get("is_process_from_selected_product"):
                        shopify_product.write(
                            {
                                "last_stock_update_date": last_export_date
                                if not shopify_product.last_stock_update_date
                                else datetime.now()
                            }
                        )
    ...
    def check_stock_type(self, instance, product_ids, prod_obj, warehouse):
        ...
        if product_ids:
            ...
            if instance.shopify_stock_field.name == "qty_available":
                ...
                prouct_listing_stock = prod_obj.get_forecated_qty(
                    warehouse, product_ids
                )
        for i in prouct_listing_stock:
            product_stock_dict.update({i.get("product_id"): i.get("stock")})
    ...
    def import_shopify_stock(self, instance):
        ...
        if templates:
            ...
            if not location_ids:
                message = "Location not found for instance %s while Import stock" % (
                    instance.name
                )
                log_line_array = self.shopify_create_log(
                    message, model_id, False, log_line_array
            ...
            for location_id in location_ids:
                ...
                shopify_location_warehouse = (
                    location_id.import_stock_warehouse_id or False
                )
                if not shopify_location_warehouse:
                    message = (
                        "No Warehouse found for Import Stock in Shopify Location: %s"
                        % (location_id.name)
                    )
                    log_line_array = self.shopify_create_log(
                        message, model_id, False, log_line_array
                ...
                try:
                    ...
                    if len(inventory_levels) >= 250:
                        inventory_levels = self.shopify_list_all_inventoryLevel(
                            inventory_levels
                        )
                except Exception as e:
                    ...
                    return False
                _logger.info(
                    "Length of the total inventory item id : %s" % len(inventory_levels)
                )
                ...
                if len(stock_inventory_array) > 0:
                    inventory = self.env["stock.inventory"].create_stock_inventory(
                        stock_inventory_array,
                    ...
                    if inventory:
                        inventory_name = (
                            'Inventory For Instance "%s" And Shopify Location "%s"'
                            % (
                                (instance.name)
                                + " "
        ...
        while result:
            ...
            for page_link in link.split(","):
                if page_link.find("next") > 0:
                    page_info = (
                        page_link.split(";")[0].strip("<>").split("page_info=")[1]
                    )
                    try:
                        result = shopify.InventoryLevel.find(
                            page_info=page_info, limit=250
                        )
                    except Exception as e:
                        if (
                            e.response.code == 429
    ...
    def shopify_create_log(
        self, message=False, model_id=False, product=False, log_line_array=False
    ):
        """
