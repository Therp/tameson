...
class ShopifyLocationEpt(models.Model):
    ...
    def _check_locations_warehouse_ids(self):
        ...
        locations = self.search(
            [("instance_id", "=", location_instance.id), ("id", "!=", self.id)]
        )
        for location in locations:
            if any(
                ...
                raise ValidationError(
                    _(
                        "Cann't set this warehouse in different locations with same instance."
                    )
                )
    ...
    def import_shopify_locations(self, instance):
        ...
        shopify_primary_location = self.search(
            [("is_primary_location", "=", True), ("instance_id", "=", instance_id)],
            limit=1,
        )
