...
class ShopifyProductTemplateEpt(models.Model):
    ...
    active_template = fields.Boolean(
        "Odoo Template Active", related="product_tmpl_id.active"
    )
    ...
    shopify_product_ids = fields.One2many(
        "shopify.product.product.ept", "shopify_template_id", "Products"
    )
    ...
    website_published = fields.Boolean(string="Published ?", default=False, copy=False)
    tag_ids = fields.Many2many(
        "shopify.tags", "shopify_tags_rel", "product_tmpl_id", "tag_id", "Tags"
    )
    description = fields.Html("Description")
    total_variants_in_shopify = fields.Integer(
        "Total Varaints", default=0
    )  # modify label name by Nilesh Parmar 20 dec 2019
    total_sync_variants = fields.Integer(
        "Total Synced Variants", compute="get_total_sync_variants", store=True
    )  # modify label name by Nilesh Parmar 20 dec 2019
    ...
    shopify_image_ids = fields.One2many(
        "shopify.product.image.ept", "shopify_template_id"
    )  # add by Bhavesh Jadav 17/12/2019 for the images

    @api.depends(
        "shopify_product_ids.exported_in_shopify", "shopify_product_ids.variant_id"
    )
    def get_total_sync_variants(self):
        ...
        for template in self:
            variants = template.shopify_product_ids.filtered(
                lambda x: x
                if x.exported_in_shopify == True and x.variant_id != False
                else False
            )
        ...
        if "active" in vals.keys():
            for shopify_template in self:
                ...
                if vals.get("active"):
                    ...
                    shopify_variants and shopify_variants.write(
                        {"active": vals.get("active")}
                    )
    ...
    @api.model
    def find_template_attribute_values(
        self, response_template, product_template, variant
    ):
        """
    ...
    def create_simple_odoo_product_template(
        self, instance, response_template, product_category, price
    ):
        """
        ...
        _logger.info(
            "Start process of simple template | shopify product(%s) and shopify product id(%s) create"
        ...
        if sku or barcode:
            ...
            available_odoo_products.update(
                {
                    response_template.get("variants")[0][
                        "id"
    ...
    def shopify_sync_products(
        ...
        if shopify_tmpl_id and not product_data_line_id:
            try:
                ...
                if e.response.code == 429 and e.response.msg == "Too Many Requests":
                    ...
                    message = (
                        "Shopify product did not exist in Shopify store with product id: %s"
                        % (shopify_tmpl_id)
                    )
                    comman_log_line_obj.shopify_create_order_log_line(
                        message, model_id, order_data_line_id, log_book_id
                    )
        ...
        if not response_template:
            return True
        _logger.info(
            "Start process of shopify product(%s) and shopify product id(%s) create"
            % (response_template.get("title"), response_template.get("id"))
        )
        ...
        for variant in response_template.get(
            "variants"
        ):  # here we are check shopify_product and odoo_product availability and prepare dict for link with template or create new variants
            shopify_product, odoo_product = self.shopify_search_odoo_product_variant(
                instance,
                variant.get("sku", ""),
                variant.get("id"),
                variant.get("barcode"),
                template_dic.get("shopify_tmpl_id"),
            )
            if shopify_product:
        ...
        for variant in response_template.get("variants"):
            ...
            if (
                not is_importable_checked
                and len(list(available_shopify_products)) > 1
                and len(list(available_shopify_products))
                == len(list(available_odoo_products))
            ):
                is_importable_checked = True

            if not is_importable_checked:
                is_importable, message = self.is_product_importable(
                    response_template, instance, odoo_product, shopify_product
                )
                if not is_importable and message:
                    if product_data_line_id:
                        comman_log_line_obj.shopify_create_product_log_line(
                            message, model_id, product_data_line_id, log_book_id
                        )
            ...
            if not shopify_product:
                if not shopify_template:
                    if not odoo_template and instance.auto_import_product:
                        ...
                        if (
                            response_template.get("options")[0].get("name") == "Title"
                            and response_template.get("options")[0].get("values")
                            == ["Default Title"]
                            and len(response_template.get("variants")) == 1
                        ):
                            (
                                odoo_template,
                                available_odoo_products,
                            ) = self.create_simple_odoo_product_template(
                                instance,
                                response_template,
                                product_category,
                                variant.get("price"),
                            ...
                            if not set_sku_barcode:
                                (
                                    odoo_template,
                                    available_odoo_products,
                                ) = shopify_product_obj.shopify_create_variant_product(
                                    response_template,
                                    instance,
                                    variant.get("price"),
                                    product_category,
                    ...
                    if not odoo_template:
                        ...
                        if order_data_line_id:
                            ...
                            is_data_line_process = True
                        break
                    shopify_template = self.create_or_update_shopify_template(
                        instance,
                        template_dic,
                        variant_info,
                        response_template,
                ...
                if not odoo_product:
                    ...
                    if odoo_template.attribute_line_ids:
                        ...
                        odoo_attributes = (
                            odoo_template.attribute_line_ids.attribute_id.ids
                        )
                        for attribute in response_template.get("options"):
                        ...
                        shopify_attributes = (
                            ...
                            .search([("id", "in", shopify_attribute_ids)])
                            .ids
                        )
                        ...
                        # Need To discuss
                        #########################################################################
                        if odoo_attributes != shopify_attributes or len(
                            odoo_attributes
                        ) != len(response_template.get("options")):
                            message = (
                                "Product %s has tried to add new attribute for sku %s in Odoo."
                            ...
                            if product_data_line_id:
                                ...
                                if instance.is_shopify_create_schedule:
                                    ...
                                    ].create_schedule_activity_for_product(
                                        queue_line=product_data_line_id, from_sale=False
                                    )
                                _logger.info(
                                    "Product {} has tried to add new attribute for sku {} of Queue {} in Odoo. Please check activity for this.".format(
                                        response_template.get("title", ""),
                                        product_sku,
                                        product_data_line_id,
                                    )
                            ...
                            for shopify_attribute in response_template.get("options"):
                                ...
                                attribute_id = (
                                    odoo_template.attribute_line_ids.filtered(
                                        lambda x: x.display_name
                                        == shopify_attribute.get("name")
                                    )
                                )
                                attrib_value_id = (
                                ...
                                attribute_line = (
                                    odoo_template.attribute_line_ids.filtered(
                                        lambda x: x.attribute_id.id
                                        == attribute_id.attribute_id.id
                                    )
                                )
                                if not attrib_value_id in attribute_line.value_ids.ids:
                                    attribute_line.value_ids = [
                                        (4, attrib_value_id, False)
                                    ]
                            template_attribute_value_domain = (
                                self.find_template_attribute_values(
                        ...
                        self.env["product.template"].create(
                            {
                                ...
                                "description_sale": variant.get("description", ""),
                            }
                        )
                        odoo_product = self.env["product.product"].search(
                            [("default_code", "=", variant["sku"])]
                ...
                shopify_product = self.env["shopify.product.product.ept"].create(
                    variant_info
                )
            else:
                if not template_updated:
                    self.create_or_update_shopify_template(
                        instance,
                        template_dic,
                        variant_info,
                        response_template,
                ...
                if (
                    not shopify_product.shopify_template_id.exported_in_shopify
                    and not shopify_product.shopify_template_id.shopify_tmpl_id
                    and shopify_template
                ):
                    variant_info.update({"shopify_template_id": shopify_template.id})
                    not_update_product.append(
                        (
                            0,
                            0,
                            {
    ...
    def shopify_sync_product_images(self, response_template):
        ...
        shopify_product_image_obj = shopify_product_images = self.env[
        ...
        for image in response_template.get("images", {}):
            if image.get("src"):
                ...
                if not variant_ids:
                                ...
                                if key in existing_common_template_images.keys():
                                    shopify_product_image = shopify_product_image_obj.create(
                                        {
                                            "shopify_template_id": self.id,
                                            "shopify_image_id": shopify_image_id,
                                            "odoo_image_id": existing_common_template_images[
                                                key
                                            ],
                                        }
                                    )
                                    ...
                                    if not self.product_tmpl_id.image_1920:
                                        ...
                                        common_product_image = (
                                            self.product_tmpl_id.ept_image_ids.filtered(
                                                lambda x: x.image
                                                == self.product_tmpl_id.image_1920
                                            )
                                        )
                                    else:
                                    ...
                                    shopify_product_image = (
                                        shopify_product_image_obj.search(
                                            [
                                                ("shopify_template_id", "=", self.id),
                                                (
                                                    "odoo_image_id",
                                    ...
                                    if shopify_product_image:
                                        shopify_product_image.shopify_image_id = (
                                            shopify_image_id
                                        )
                        except Exception:
                            pass
                    ...
                    """For Variant Images."""
                    shopify_products = self.shopify_product_ids.filtered(
                        lambda x: int(x.variant_id) in variant_ids
                    )
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if key in existing_common_variant_images.keys():
                                        shopify_product_image = shopify_product_image_obj.create(
                                            {
                                                ...
                                                "shopify_variant_id": shopify_product.id,
                                                "shopify_image_id": shopify_image_id,
                                                "odoo_image_id": existing_common_variant_images[
                                                    key
                                                ],
                                            }
                                        )
                                        ...
                                        if (
                                            not shopify_product.product_id.image_1920
                                            or not is_template_image_set
                                        ):
                                            shopify_product.product_id.image_1920 = (
                                                image
                                            )
        ...
        _logger.info("Images Updated for shopify {}".format(self.name))
        return True
    ...
    def update_variant_price(self, shopify_product, variant_price):
        pricelist_item = self.env["product.pricelist.item"].search(
            [
                (
                    "pricelist_id",
                    "=",
                    shopify_product.shopify_instance_id.shopify_pricelist_id.id,
                ),
                ("product_id", "=", shopify_product.product_id.id),
            ],
            limit=1,
        ...
        if pricelist_item:
            if (
                ...
                != shopify_product.shopify_template_id.product_tmpl_id.company_id.currency_id.id
            ):
                variant_price = pricelist_item.currency_id.compute(
                    float(variant_price),
    ...
    def prepare_shopify_template_variant_comman_vals(
        self, instance, shopify_template, template_dic, variant_dic, variant_sequence
    ):
    ...
    def convert_shopify_date_into_odoo_format(self, product_date):
        ...
        shopify_product_date = (
            parser.parse(product_date).astimezone(utc).strftime("%Y-%m-%d %H:%M:%S")
        )
        return shopify_product_date

    def shopify_search_odoo_product_variant(
        self, shopify_instance, product_sku, variant_id, barcode, s_template_id
    ):
        """
        ...
        if shopify_instance.shopify_sync_product_with == "sku" and product_sku:
            if not shopify_product:
                shopify_product = shopify_product_obj.search(
                    [
                        ("default_code", "=", product_sku),
                        ("shopify_instance_id", "=", shopify_instance.id),
                        ("shopify_template_id.shopify_tmpl_id", "=", s_template_id),
                    ],
                    limit=1,
                )
            if not shopify_product:
            ...
            if not shopify_product:
                odoo_product = odoo_product.search(
    ...
    def create_or_update_shopify_template(
        self,
        instance,
        template_dict,
        variant_dic,
        ...
        if not odoo_product and not shopify_product:
            ...
            for product_sku_barcode in product_sku_barcodes:
                ...
                if not sku and not barcode:
                    message = (
                        "From the data received of the Shopify store, SKU(s) or  "
                        "Barcode(s) "
                    ...
                    if product_count == 1:
                        message = (
                            "From the data received of the Shopify store, SKU or Barcode is not set in Shopify Product: '%s' and ID: "
                ...
                if barcode:
                    duplicate_barcode = self.env["product.product"].search(
                        [("barcode", "=", barcode)]
                    )
                    if duplicate_barcode:
                        message = (
                            "Duplicate barcode found in Product: %s and ID: %s."
                            % (template_title, template_id)
                        )
                        importable = False
                        return importable, message
        ...
        if odoo_product:
            ...
            if not (product_count == 1 and odoo_template.product_variant_count == 1):
                if product_count == odoo_template.product_variant_count:
                    for shopify_prdct, odoo_prdct in zip(
                        result.get("variants"), odoo_template.product_variant_ids
                    ):
                    ...
                    shopify_barcodes = list(
                        filter(lambda x: len(x) > 0, shopify_barcodes)
                    )
                    ...
                    for product_sku_barcode in product_sku_barcodes:
                        ...
                        if not sku and not barcode:
                            message = (
                                "All SKU(s) or Barcode(s) are not set in Product: %s and ID: %s."
                                % (template_title, template_id)
                            )
                            importable = False
                            return importable, message
                    ...
                    for sku in shopify_skus:
                        if sku not in odoo_skus:
                            message = (
                                "SKU not found in Odoo for Product: %s and SKU: %s."
                                % (template_title, sku)
                            )
                            importable = False
                            return importable, message
                    for barcode in shopify_barcodes:
                        if barcode not in odoo_barcodes:
                            message = (
                                "Barcode not found in Odoo for Product: %s and Barcode: %s."
            ...
            for variantion in variants:
                ...
                if barcode:
                    ...
                    if duplicate_barcode:
                        ...
                        if (
                            shopify_variant
                            and shopify_variant.product_id
                            and shopify_variant.product_id.id != duplicate_barcode.id
                        ):
                            message = (
                                "Duplicate barcode found in Product: %s and ID: %s."
                                % (template_title, template_id)
            ...
            try:
                ...
                if new_product:
                    ...
                    if self._context.get("publish") == "shopify_unpublish":
                        ...
                        new_product.published_at = datetime.utcnow().strftime(
                            "%Y-%m-%dT%H:%M:%S"
                        )
                ...
                message = "Template %s not found in shopify When Publish" % (
                    self.shopify_tmpl_id
                )
                vals = {
                    "message": message,
                    "model_id": model_id,
