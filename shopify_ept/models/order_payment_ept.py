...
class ShopifyOrderPayment(models.Model):
    ...
    workflow_id = fields.Many2one(
        "sale.workflow.process.ept", string="Auto Sales Workflow", copy=False
    )
    payment_gateway_id = fields.Many2one(
        "shopify.payment.gateway.ept", string="Payment Gateway", copy=False
    )
    amount = fields.Float()
    remaining_refund_amount = fields.Float(
        help="Remaining refund amount in Shopify Store."
    )
    payment_transaction_id = fields.Char(help="It is used for a refund.")
    is_fully_refunded = fields.Boolean(
        help="It is to identify that it is fully refunded in the Shopify store.",
        default=False,
    )
    ...
    def _onchange_refund_amount(self):
        ...
        if self.refund_amount > self.remaining_refund_amount:
            raise UserError(
                _("The Refund Amount should be less than of Remaining Refund Amount.")
            )
