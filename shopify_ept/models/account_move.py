...
class AccountMove(models.Model):
    ...
    def refund_in_shopify(self):
        ...
        if self.reversed_entry_id.is_shopify_multi_payment:
            payment_gateway_ids = (
                self.reversed_entry_id.invoice_line_ids.sale_line_ids.order_id.shopify_payment_ids
            )
            payment_gateway_ids.write(
                {"refund_amount": 0.0, "is_want_to_refund": False}
            )
            remaining_to_refund_payment_ids = payment_gateway_ids.filtered(
                lambda payment: payment.is_fully_refunded == False
            ).payment_gateway_id.ids
            context.update(
                {
                    "display_refund_from": False,
                    "payment_gateway_ids": remaining_to_refund_payment_ids,
    ...
    def open_shopify_multi_payment(self):
        ...
        view_id = self.env.ref(
            "shopify_ept.shopify_multi_payment_gateway_tree_view_ept"
        ).id
        return {
            ...
            "domain": [
                (
                    "id",
                    "in",
                    self.line_ids.sale_line_ids.order_id.shopify_payment_ids.ids,
                )
            ],
            "target": "new",
            "context": context,
