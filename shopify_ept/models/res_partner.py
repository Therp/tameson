...
class res_partner(models.Model):
    ...
    def create_shopify_pos_customer(self, order_response, log_book_id, instance):
        ...
        first_name = (
            customer_data.get("first_name") if customer_data.get("first_name") else ""
        )
        last_name = (
            customer_data.get("last_name") if customer_data.get("last_name") else ""
        )
        ...
        if shopify_partner:
            ...
            if res_partner:
                ...
                if res_partner:
                    ...
                    partner_vals.update(
                        {"is_shopify_customer": True, "type": "contact"}
                    )
                    res_partner = self.create(partner_vals)

            shopify_partner_obj.create(
                {
                    "shopify_instance_id": instance.id,
                    "shopify_customer_id": customer_id,
    ...
    def create_or_update_customer(
        ...
        if is_company:
            if order_data_queue_line:
                ...
                if not address:
                    message = (
                        "Customer is skip because of the address was not found please verify customer in shopify "
                        "store"
                    )
            ...
            if customer_data_queue_line_id:
                ...
                if vals.get("default_address"):
                    ...
                    comman_log_line_obj.shopify_create_customer_log_line(
                        message, model_id, customer_data_queue_line_id, log_book_id
                    )
                    customer_data_queue_line_id and customer_data_queue_line_id.write(
                        {"state": "failed", "last_process_date": datetime.now()}
                    )

                    return False
            company_name = address.get("company")
            name = address.get("name") or "%s %s" % (
            ...
            update_partner_vals = self.remove_special_chars_from_partner_vals(
                partner_vals
            )
            ...
            if res_partner_id:
                ...
                res_partner = self.shopify_search_partner_with_and_without_company_name(
                    key_list, company_name, partner_vals
                )
                if not res_partner:
                    key_list = [
                        "name",
                    ...
                    res_partner = (
                        self.shopify_search_partner_with_and_without_company_name(
                            key_list, company_name, partner_vals
                        )
                    )
                    if not res_partner:
            ...
            address_child = self.shopify_search_partner_with_and_without_company_name(
                key_list, company_name, partner_vals
            )
    ...
    def shopify_search_partner_with_and_without_company_name(
        self, key_list, company_name, partner_vals
    ):
        """This method is used to search partner with company name and without company name.
        @return: res_partner
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 23 November 2020 .
        Task_id: 168376
        """
        res_partner = False
        ...
        if not res_partner and company_name:
            res_partner = self._find_partner(
                partner_vals, key_list, [("company_name", "=", False)]
            )
            if res_partner:
                res_partner.company_name = company_name
