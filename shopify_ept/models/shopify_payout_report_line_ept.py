...
class ShopifyPayoutReportLineEpt(models.Model):
    ...
    payout_id = fields.Many2one(
        "shopify.payout.report.ept", string="Payout ID", ondelete="cascade"
    )
    transaction_id = fields.Char(
        string="Transaction ID", help="The unique identifier of the " "transaction."
    )
    source_order_id = fields.Char(
        string="Order Reference ID",
        help="The id of the Order that "
        "this transaction"
    ...
    fee = fields.Float(
        string="Fees",
        help="The total amount of fees deducted from the " "transaction amount.",
    )
