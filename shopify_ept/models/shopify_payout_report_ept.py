...
class ShopifyPaymentReportEpt(models.Model):
    ...
    payout_reference_id = fields.Char(
        string="Payout Reference ID", help="The unique identifier of the payout"
    )
    payout_date = fields.Date(
        string="Payout Date", help="The date the payout was issued."
    )
    payout_transaction_ids = fields.One2many(
        "shopify.payout.report.line.ept", "payout_id", string="Payout transaction lines"
    )
    reconciliation_log_ids = fields.One2many(
        "shopify.payout.logline.ept", "payout_id", string="Transaction log lines"
    )
    ...
    state = fields.Selection(
        [
            ("draft", "Draft"),
            ("partially_generated", "Partially Generated"),
            ("generated", "Generated"),
    ...
    is_skip_from_cron = fields.Boolean(
        string="Skip From Schedule Actions", default=False
    )
    ...
    def check_process_statement(self):
        ...
        if self.payout_transaction_ids:
            if any(
                line.is_remaining_statement == True
                for line in self.payout_transaction_ids
            ):
    ...
    def generate_remaining_bank_statement(self):
        ...
        remaining_transaction_ids = self.payout_transaction_ids.filtered(
        ...
        for transaction in remaining_transaction_ids:
            ...
            if transaction.transaction_type == "charge":
                invoice_ids = order_id.invoice_ids.filtered(
                    lambda l: l.state == "posted" and l.type == "out_invoice"
                )
                if not invoice_ids:
                    continue
            if transaction.transaction_type == "refund":
                invoice_ids = order_id.invoice_ids.filtered(
                    lambda l: l.state == "posted" and l.type == "out_refund"
                )
                if not invoice_ids:
                    continue
            ...
            account_id = (
                shopify_account_config_id and shopify_account_config_id.account_id
            )
            if transaction.transaction_type == "charge":
                payment_reference = self.env["account.payment"].search(
                    [
    ...
    def generate_bank_statement(self):
        ...
        if not journal:
            message_body = (
                "You have not configured Payout report Journal in "
        ...
        currency_id = (
            ...
            or self.instance_id.shopify_company_id.currency_id.id
            or False
        )
        ...
        name = "{}_{}".format(self.instance_id.name, self.payout_reference_id)
        vals = {
        ...
        for transaction in self.payout_transaction_ids:
            order_id = transaction.order_id
            if (
                transaction.transaction_type in ["charge", "refund", "payment_refund"]
                and not order_id
            ):
                ...
                if order_id:
                    ...
                    message = (
                        "Transaction line {} will not automatically reconcile due to "
                        "order {} is not found in odoo.".format(
                            transaction.transaction_id, transaction.source_order_id
                        )
                    )
                    payout_logline_obj.create(
                        {
                            "message": message,
                    ...
                    bank_line_vals = self.prepare_bank_statement_line_vals(
                        transaction, reference, partner, bank_statement_id
                    )
                    bank_statement_line_obj.create(bank_line_vals)
            ...
            if transaction.transaction_type == "charge":
                invoice_ids = order_id.invoice_ids.filtered(
                    ...
                    and l.type == "out_invoice"
                    and l.amount_total == transaction.amount
                )
                if not invoice_ids:
                    message = (
                        "Transaction line {} will not automatically reconcile due to "
                        "Invoice is not found in odoo.".format(
                    ...
                    transaction.is_remaining_statement = False
                    continue
            if transaction.transaction_type in ["refund", "payment_refund"]:
                invoice_ids = order_id.invoice_ids.filtered(
                    lambda l: l.state == "posted"
                    and l.type == "out_refund"
                    and l.amount_total == -(transaction.amount)
                )
                if not invoice_ids:
                    message = (
                        "Transaction line {} will not automatically reconcile due to "
                        "refund is not found in odoo.".format(
                            order_id.name or transaction.source_order_id
                        )
                    )
                    payout_logline_obj.create(
            ...
            if transaction.transaction_type not in [
                ...
                shopify_account_config_id = (
                    self.instance_id.transaction_line_ids.filtered(
                        lambda l: l.transaction_type == transaction.transaction_type
                    )
                )
                account_id = (
        ...
        if self.reconciliation_log_ids:
            self.env["shopify.payout.logline.ept"].create_payout_schedule_activity(
                self.reconciliation_log_ids
            )
        self.is_skip_from_cron = False
        return True

    def prepare_bank_statement_line_vals(
        self, transaction, reference, partner, bank_statement_id
    ):
        """This method is used to prepare a bank statement line vals based on the transaction line.
        @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 23 March 2021 .
        Task_id: 171975 - Shopify Payout changes
        """
        line_vals = {
            "name": transaction.transaction_id or transaction.transaction_type,
            "ref": reference or "",
            "date": self.payout_date,
            "partner_id": partner and partner.id,
            "amount": transaction.amount,
    ...
    def convert_move_amount_currency(self, bank_statement, moveline, amount):
        ...
        if moveline.company_id.currency_id.id != bank_statement.currency_id.id:
            ...
            amount_currency = moveline.currency_id.compute(
                moveline.amount_currency, bank_statement.currency_id
            )
        elif (
            moveline.move_id
            and moveline.move_id.currency_id.id != bank_statement.currency_id.id
        ):
            amount_currency = moveline.move_id.currency_id.compute(
                amount, bank_statement.currency_id
            )
    ...
    def process_bank_statement(self):
        ...
        _logger.info("Processing Bank Statement: {}.".format(bank_statement.name))
        for statement_line in bank_statement.line_ids.filtered(
            lambda x: x.journal_entry_ids.ids == []
        ):
            try:
                mv_list = []
                payment_aml_rec = []
                ...
                if ref:
                    ...
                    if statement_line.account_ept_id:
                        mv_dicts = {
                            ...
                            "debit": statement_line.amount < 0
                            and -statement_line.amount
                            or 0.0,
                            "credit": statement_line.amount > 0
                            and statement_line.amount
                            or 0.0,
                        }
                ...
                if not payment_aml_rec and not mv_list:
                    for order in statement_line.shopify_order_ids:
                        if statement_line.amount < 0.0:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == "out_refund"
                                and record.state == "posted"
                            )
                        else:
                            invoices += order.invoice_ids.filtered(
                                lambda record: record.type == "out_invoice"
                                and record.state == "posted"
                            )
                    for invoice in invoices:
                        if invoice.state == "posted":
                            ...
                            if payment_ids:
                                payment_aml_rec = payment_ids.mapped(
                                    "move_line_ids"
                                ).filtered(
                                ...
                                for moveline in move_lines:
                                    ...
                                    if moveline.amount_currency:
                                        (
                                            currency,
                                            amount_currency,
                                        ) = self.convert_move_amount_currency(
                                ...
                                if round(statement_line.amount, 10) == round(
                                    move_line_total_amount, 10
                                ) and (
                                    not statement_line.currency_id
                                    or statement_line.currency_id.id
                                    == bank_statement.currency_id.id
                                ):
                                    if currency_ids:
                                        ...
                                        if len(currency_ids) == 1:
                                            statement_line.write(
                                                {
                                                    "amount_currency": move_line_total_amount,
                    ...
                    for aml_dict in mv_line_dicts:
                        if aml_dict["move_line"].reconciled:
                            ...
                            _logger.info(
                                "Statement line is already reconciled: {}".format(
                ...
                statement_line.process_reconciliation(
                    counterpart_aml_dicts=mv_line_dicts,
                    payment_aml_rec=payment_aml_rec,
                    new_aml_dicts=mv_list,
                )
                _logger.info(
                    "Statement reconciled for Reference: {}, Label: {}, Amount: {}.".format(
                        statement_line.ref or "",
                        statement_line.name or "",
                        statement_line.amount,
                    )
                )
            except Exception as error:
                message = "statement line occurred while reconciliation : {}.".format(
                    error
                )
                payout_logline_obj.create(
                    {
    ...
    def get_payout_report(self, start_date, end_date, instance):
        ...
        try:
            ...
            message = "Something is wrong while import the payout records : {}".format(
                error
            )
            if not log_book_id:
        ...
        for payout in payout_ids:
            ...
            for transaction in transaction_ids:
                ...
                transaction_vals = self.prepare_transaction_vals(
                    transaction, payout_id, instance
                )
                shopify_payout_report_line_obj.create(transaction_vals)
            # Create fees line
            fees_amount = (
                float(payout.get("summary").get("charges_fee_amount", 0.0))
                + float(payout.get("summary").get("refunds_fee_amount", 0.0))
                + float(payout.get("summary").get("adjustments_fee_amount", 0.0))
            )
    ...
    def closed_statement(self):
        ...
        try:
            ...
            message_body = "There is some issue in close statement: Error: {} ".format(
                e
            )
            log_line_id = payout_logline_obj.search(
                [
                    ("payout_id", "=", self.id),
                    ("instance_id", "=", self.instance_id.id),
            ...
            if not log_line_id:
                log_line_id = payout_logline_obj.create(
                    {
                        "message": message_body,
                        "instance_id": self.instance_id.id,
                        "payout_id": self.id,
            ...
            self.env["shopify.payout.logline.ept"].with_context(
                {"is_closed": True}
            ).create_payout_schedule_activity(log_line_id)
            self.is_skip_from_cron = True
            return False
        self.state = "closed"
        return True
    ...
    def auto_import_payout_report(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            if instance.payout_last_import_date:
                ...
                payout_report_obj.get_payout_report(
                    start_date=instance.payout_last_import_date,
    ...
    def auto_generate_bank_statement(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            for draft_report in draft_reports:
                _logger.info(
                    "cron =====:Auto Generate Bank Statement: {} =====".format(
                        draft_report.name
                    )
                )
            ...
            for report in partially_generated_report:
                _logger.info(
                    "Cron : +++ Partially processed report : {}".format(report.name)
                )
    ...
    def auto_process_bank_statement(self, ctx={}):
        ...
        if shopify_instance_id:
            ...
            for generated_report in generated_reports:
                _logger.info(
                    "cron =====:Auto Process Bank Statement: {} =====".format(
