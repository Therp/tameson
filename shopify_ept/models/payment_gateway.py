...
class ShopifyPaymentGateway(models.Model):
    ...
    shopify_instance_id = fields.Many2one(
        "shopify.instance.ept", required=True, string="Instance"
    )
    ...
    def search_or_create_payment_gateway(self, instance, gateway_name):
        ...
        shopify_payment_gateway = self.search(
            [("code", "=", gateway_name), ("shopify_instance_id", "=", instance.id)],
            limit=1,
        )
    ...
    def shopify_search_create_gateway_workflow(
        self, instance, order_data_queue_line, order_response, log_book_id, gateway
    ):
        ...
        if not workflow_config:
            message = (
                "Workflow Configuration not found for this order %s and payment gateway is "
        ...
        if auto_workflow_id and not auto_workflow_id.picking_policy:
            message = (
                "Please set the proper auto workflow configuration and name is : %s. You "
                "can see the auto "
