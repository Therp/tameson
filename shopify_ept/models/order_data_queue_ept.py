...
class ShopifyOrderDataQueueEpt(models.Model):
    ...
    shopify_order_common_log_book_id = fields.Many2one(
        "common.log.book.ept",
        help="""Related Log book which has
    ...
    order_data_queue_line_ids = fields.One2many(
        "shopify.order.data.queue.line.ept", "shopify_order_data_queue_id"
    )
    ...
    created_by = fields.Selection(
        [
            ("import", "By Manually Import Process"),
            ("webhook", "By Webhook"),
    ...
    is_action_require = fields.Boolean(
        default=False, help="it is used  to find the action require queue"
    )
    ...
    def _compute_queue_state(self):
        ...
        for record in self:
            if (
                record.order_queue_line_total_record
                == record.order_queue_line_done_record
                + record.order_queue_line_cancel_record
            ):
                record.state = "completed"
            elif (
                record.order_queue_line_draft_record
                == record.order_queue_line_total_record
            ):
                record.state = "draft"
            elif (
                record.order_queue_line_total_record
                == record.order_queue_line_fail_record
            ):
                record.state = "failed"
            else:
    ...
    def import_order_cron_action(self, ctx={}):
        ...
        self.shopify_create_order_data_queues(
            instance, from_date, to_date, created_by="scheduled_action"
        )

    def shopify_create_order_data_queues(
        self, instance, from_date, to_date, created_by="import"
    ):
        ...
        for order_status_id in instance.import_shopify_order_status_ids:
            ...
            if (
                shopify_fulfillment_status == "any"
                or shopify_fulfillment_status == "shipped"
            ):
                ...
                order_queues = order_data_queue_line_boj.create_order_data_queue_line(
                    order_ids, instance, created_by=created_by
                )
                ...
                if len(order_ids) >= 50:
                    order_ids = self.list_all_orders(order_ids, instance, created_by)
        _logger.info(
            "Length of Shopify orders %s import from instance name: %s"
            ...
            for page_link in link.split(","):
                if page_link.find("next") > 0:
                    ...
                    try:
                        ...
                        order_data_queue_line_boj.create_order_data_queue_line(
                            result, instance, created_by=created_by
                        )
                        self._cr.commit()
    ...
    def create_schedule_activity(self, queue_id):
        ...
        if queue_id:
            ...
            if shopify_order_id_list and len(shopify_order_id_list) > 0:
                note = (
                    "Your order has not been imported for Shopify Order Reference : %s"
                    % str(shopify_order_id_list)[1:-1]
                )
    ...
    def import_order_process_by_remote_ids(self, instance, order_ids):
        ...
        if order_ids:
            ...
            if len(order_ids.split(",")) <= 50:
                ...
                results = shopify.Order().find(
                    ids=",".join(order_ids_list), status="any"
                )
                if results:
                    _logger.info(
                        "Length of Shopify orders %s import from instance name: %s"
                    ...
                    [
                        order_ids_list.remove(str(result.id))
                        for result in results
                        if str(result.id) in order_ids_list
                    ]
            else:
                raise Warning(_("Please enter the Order ids 50 or less"))
            if results:
                order_queues = self.env[
                    "shopify.order.data.queue.line.ept"
                ].create_order_data_queue_line(results, instance)
                ...
                if order_data_queue:
                    if order_ids_list or only_alphabets:
                        order_data_queue.message_post(
                            body="%s orders are not imported"
                            % (",".join(order_ids_list + only_alphabets))
                        )
                    order_data_queue.order_data_queue_line_ids.process_import_order_queue_data()
                    _logger.info(
                        "Imported order queue : {} of Instance : {} via Import shopify Remote Order ids Successfully".format(
                            order_data_queue.name, instance.name
                        )
                    )
                _logger.info(
                    "Processed order queue : {} of Instance : {} via Import Remote Order Ids Successfully".format(
                        order_data_queue.name, instance.name
                    )
                )
