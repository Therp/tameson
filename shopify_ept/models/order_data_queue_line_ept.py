...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    shopify_order_data_queue_id = fields.Many2one(
        "shopify.order.data.queue.ept", ondelete="cascade"
    )
    ...
    sale_order_id = fields.Many2one(
        "sale.order", copy=False, help="Order created in Odoo."
    )
    order_data = fields.Text(
        help="Data imported from Shopify of current order.", copy=False
    )
    # shopify_product = fields.Text(help="Shopify Product Name", copy=False)
    customer_name = fields.Text(
        string="Customer Name", help="Shopify Customer Name", copy=False
    )

    customer_email = fields.Text(
        string="Customer Email", help="Shopify Customer Email", copy=False
    )
    ...
    def search_webhook_order_queue(self, created_by, instance, order, one_time_create):
        shopify_order_queue_obj = self.env["shopify.order.data.queue.ept"]

        order_queue = shopify_order_queue_obj.search(
            [
                ("created_by", "=", created_by),
                ("state", "in", ["draft", "partially_completed"]),
        ...
        if order_queue:
            _logger.info(
                "Order %s added into Order Queue %s."
                % (order.get("name"), order_queue.name)
            )
            one_time_create = False
        return order_queue, one_time_create
    ...
    def shopify_create_order_queue(self, instance, created_by="import"):
        ...
        order_queue_data_id = self.env["shopify.order.data.queue.ept"].create(
            oredr_queue_vals
        )
    ...
    def auto_start_child_process_for_order_queue(self):
        ...
        """
        self.auto_import_order_queue_data()
        return True
    ...
    def auto_import_order_queue_data(self):
        ...
        order_data_queue_obj = self.env["shopify.order.data.queue.ept"]
        ...
        start = time.time()
        queue = order_data_queue_obj.browse(order_queue_ids[0])
        order_queue_process_cron_time = (
            queue.shopify_instance_id.get_shopify_cron_execution_time(
                "shopify_ept.ir_cron_parent_to_process_order_queue_data"
            )
        )
        for order_queue_id in order_queue_ids:
            queue = order_data_queue_obj.browse(order_queue_id)
            order_data_queue_line_ids = queue.order_data_queue_line_ids.filtered(
            ...
            if queue.queue_process_count > 3:
                ...
                if queue.shopify_instance_id.is_shopify_create_schedule:
                    model_id = (
                        self.env["ir.model"]
                        .search([("model", "=", "shopify.order.data.queue.ept")])
                        .id
                    )
    ...
    def process_import_order_queue_data(self, update_order=False):
        ...
        queue_id = (
            self.shopify_order_data_queue_id
            if len(self.shopify_order_data_queue_id) == 1
            else False
        )
        if queue_id:
            ...
            for order_queue_line in self:
                ...
                if update_order:
                    orders = self.env["sale.order"].update_shopify_order(
                        order_queue_line, log_book_id
                    )
                else:
            ...
            if (
                queue_id.shopify_order_common_log_book_id
                and not queue_id.shopify_order_common_log_book_id.log_lines
            ):
                queue_id.shopify_order_common_log_book_id.unlink()
            if queue_id.shopify_instance_id.is_shopify_create_schedule:
                self.env["shopify.order.data.queue.ept"].create_schedule_activity(
                    queue_id
                )
            return True
