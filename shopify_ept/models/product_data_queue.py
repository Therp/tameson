...
class ShopifyProductDataqueue(models.Model):
    ...
    product_data_queue_lines = fields.One2many(
    ...
    queue_line_total_records = fields.Integer(
        string="Total Records", compute="_compute_queue_line_record"
    )
    queue_line_draft_records = fields.Integer(
        string="Draft Records", compute="_compute_queue_line_record"
    )
    queue_line_fail_records = fields.Integer(
        string="Fail Records", compute="_compute_queue_line_record"
    )
    queue_line_done_records = fields.Integer(
        string="Done Records", compute="_compute_queue_line_record"
    )
    queue_line_cancel_records = fields.Integer(
        string="Cancelled Records", compute="_compute_queue_line_record"
    )
    ...
    def _compute_queue_state(self):
        ...
        for record in self:
            if (
                record.queue_line_total_records
                == record.queue_line_done_records + record.queue_line_cancel_records
            ):
    ...
    def shopify_create_product_data_queue(self, instance, template_ids=""):
        ...
        if template_ids:
            ...
            if len(template_ids.split(",")) <= 50:
                ...
                if results:
                    _logger.info(
                        "Length of Shopify Products %s import from instance name: %s"
                    ...
                    [
                        template_ids.remove(str(result.id))
                        for result in results
                        if str(result.id) in template_ids
                    ]
            else:
            ...
            if not instance.shopify_last_date_product_import:
                ...
                results = shopify.Product().find(
                    status="active",
                    updated_at_min=instance.shopify_last_date_product_import,
                    limit=250,
                )  # Change by bhavesh jadav 13/12/2019 limit=250
                if len(results) >= 250:
                    results = self.shopify_list_all_products(results)
            if results:
                ...
                for result in results:
                    if (
                        result.to_dict().get("variants")[0].get("fulfillment_service")
                        != "gift_card"
                    ):
        ...
        for result in results:
            if one_time_create:
                ...
                _logger.info(
                    "Shopify Product Queue created. Queue name is  {}".format(
                ...
                if template_ids or only_alphabets:
                    product_queue_id.message_post(
                        body="%s products are not imported"
                        % (",".join(template_ids + only_alphabets))
                    )
            self.shopify_create_product_data_queue_line(
                result, instance, product_queue_id
            )
            count = count + 1
            if count == 100:
                count = 0
    ...
    def shopify_list_all_updated_products(self, result, updated_at_min):
        ...
        try:
            new_result = shopify.Product().find(
                limit=250, updated_at_min=updated_at_min, page=2
            )
        ...
        while new_result:
            ...
            try:
                new_result = shopify.Product().find(
                    limit=250, updated_at_min=updated_at_min, page=page_no
                )
            except Exception as e:
                if e.response.code == 429 and e.response.msg == "Too Many Requests":
                    time.sleep(int(float(e.response.headers.get("Retry-After", 5))))
    ...
    def shopify_create_product_data_queue_line(
        self, result, instance, product_queue_data_id
    ):
        ...
        product_queue_line_vals.update(
            {
                "product_data_id": result.get("id"),
                "shopify_instance_id": instance and instance.id or False,
                "synced_product_data": data,
                "name": result.get("title"),
                "product_data_queue_id": product_queue_data_id
                and product_queue_data_id.id
                or False,
                "state": "draft",
            }
        )
    ...
    def create_schedule_activity_for_product(self, queue_line, from_sale=False):
        ...
        if from_sale:
            ...
            model_id = ir_model_obj.search(
                [("model", "=", "shopify.order.data.queue.ept")]
            )
            data_ref = queue_line.shopify_order_id
            note = (
                "Your order has not been imported because of the product of order Has a new attribute  Shopify Order "
            ...
            note = (
                "Your product was not synced because you tried to add new attribute | Product Data Reference : %s"
                % data_ref
            )
        activity_type_id = (
    ...
    def create_product_queue_from_webhook(self, product_data, instance):
        ...
        if product_data_queue:
            _logger.info(
                "Product %s added into Queue %s."
                % (product_data.get("id"), product_data_queue.name)
            )
        else:
            product_data_queue = self.shopify_create_product_queue(instance, "webhook")
            _logger.info(
                "Imported product {} of {} via Webhook Successfully.".format(
                    product_data.get("id"), instance.name
                )
            )

        self.shopify_create_product_data_queue_line(
            product_data, instance, product_data_queue
        )
        ...
        if len(product_data_queue.product_data_queue_lines) >= 50:
            ...
            _logger.info(
                "Processed product {} of {} via Webhook Successfully.".format(
                    product_data.get("id"), instance.name
                )
            )
