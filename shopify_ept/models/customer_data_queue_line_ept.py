...
class ShopifyCustomerDataQueueLineEpt(models.Model):
    ...
    synced_customer_queue_id = fields.Many2one(
        "shopify.customer.data.queue.ept", string="Shopify Customer", ondelete="cascade"
    )
    ...
    def sync_shopify_customer_into_odoo(self, results=False):
        ...
        if not results:
            ...
            start = time.time()
            customer_queue_process_cron_time = (
            ...
            for queue_id in customer_queue_ids:
                ...
                if queue.queue_process_count > 3:
                    ...
                    if queue.shopify_instance_id.is_shopify_create_schedule:
                        model_id = (
                            self.env["ir.model"]
                            .search([("model", "=", "shopify.customer.data.queue.ept")])
                            .id
                        )
                        common_log_obj.create_crash_queue_schedule_activity(
                            queue, model_id, note
                        )
                    return
                ...
                if not queue.shopify_instance_id.active:
                    _logger.info(
                        "Instance '{}' is not active.".format(
                            queue.shopify_instance_id.name
                        )
                    )
                    return True
                ...
                for line in results:
                    ...
                    partner = partner_obj.create_or_update_customer(
                        vals=json.loads(line.shopify_synced_customer_data),
                        is_company=True,
                        parent_id=False,
                        type=False,
                        instance=instance,
                        customer_data_queue_line_id=line,
    ...
    def create_customer_queue_schedule_activity(self, queue_id):
        ...
        model_id = ir_model_obj.search(
            [("model", "=", "shopify.customer.data.queue.ept")]
        )
        activity_type_id = (
