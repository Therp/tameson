...
class SaleAutoWorkflowConfiguration(models.Model):
    ...
    auto_workflow_id = fields.Many2one(
        "sale.workflow.process.ept", "Auto Workflow", required=1
    )
    payment_gateway_id = fields.Many2one(
        "shopify.payment.gateway.ept", "Payment Gateway", required=1
    )
    ...
    def create_financial_status(self, instance, financial_status):
        ...
        """
        payment_methods = self.env["shopify.payment.gateway.ept"].search(
            [("shopify_instance_id", "=", instance.id)]
        )
        auto_workflow_record = self.env.ref(
            "auto_invoice_workflow_ept.automatic_validation_ept"
        )
