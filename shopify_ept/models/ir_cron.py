...
class IrCron(models.Model):
    ...
    def update_existing_cron_for_shopify_ept(self):
        ...
        if not config_param:
            product_parent_cron = self.env.ref(
                "shopify_ept.ir_cron_parent_to_process_product_queue_line", False
            )
            order_parent_cron = self.env.ref(
                "shopify_ept.ir_cron_parent_to_process_order_queue_data", False
            )
            customer_parent_cron = self.env.ref(
            ...
            if product_parent_cron and product_parent_cron.interval_number < 5:
                product_parent_cron.write(
                    {"interval_number": 5, "name": "Shopify: Process Product Queue"}
                )

            if order_parent_cron and order_parent_cron.interval_number < 5:
                order_parent_cron.write(
                    {"interval_number": 5, "name": "Shopify: Process Order Queue"}
                )

            if customer_parent_cron and customer_parent_cron.interval_number < 5:
                customer_parent_cron.write(
                    {"interval_number": 5, "name": "Shopify: Process Customer Queue"}
                )
