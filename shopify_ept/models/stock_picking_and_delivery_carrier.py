...
class DeliveryCarrier(models.Model):
    ...
    shopify_source = fields.Char(
        "Shopify Delivery Source",
        help="when system will import order then search carrier based on shopift_tracking_company if not found thne search based on source.",
    )
    ...
    def shopify_search_create_delivery_carrier(self, line, instance):
        ...
        if delivery_source and delivery_code:
            carrier = self.search(
                [
                    ("shopify_source", "=", delivery_source),
                    "|",
                    ("shopify_code", "=", delivery_code),
                ...
                carrier = self.create(
                    {
                        ...
                        if instance.shopify_shipping_product_id
                        else product_template.product_variant_ids[0].id,
                    }
                )
        return carrier
