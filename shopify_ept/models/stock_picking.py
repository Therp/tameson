...
class stock_picking(models.Model):
    ...
    shopify_instance_id = fields.Many2one(
        "shopify.instance.ept", "Instance", store=True
    )
    is_cancelled_in_shopify = fields.Boolean(
        "Is Cancelled In Shopify ?",
        default=False,
        copy=False,
        help="Use this field to identify shipped in Odoo but cancelled in Shopify",
    )
    is_manually_action_shopify_fulfillment = fields.Boolean(
        "Is Manually Action Required ?",
        default=False,
        copy=False,
