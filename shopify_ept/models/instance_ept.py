...
class ShopifyInstanceEpt(models.Model):
    ...
    def _get_default_warehouse(self):
        warehouse = self.env["stock.warehouse"].search(
            [("company_id", "=", self.env.company.id)], limit=1, order="id"
        )
    ...
    def _count_all(self):
        for instance in self:
            ...
            product_query = self.prepare_query_to_count_record(
                "shopify_product_template_ept", instance
            )
            instance.product_count = self.query_to_product_count(product_query)
            instance.exported_product_count = self.query_to_product_count(
                False, instance, "exported_in_shopify", "true"
            )
            instance.ready_to_expor_product_count = self.query_to_product_count(
                False, instance, "exported_in_shopify", "false"
            )
            instance.published_product_count = self.query_to_product_count(
                False, instance, "website_published", "true"
            )
            unpublished_product_query = (
                product_query
                + " and website_published = 'false' and exported_in_shopify = 'true'"
            )
            instance.unpublished_product_count = self.query_to_product_count(
                unpublished_product_query
            )
            ...
            instance.quotation_count = self.query_to_sale_order_count(
                False, instance, "state", ("draft", "sent")
            )
            order_query = sale_query + " and state not in ('draft', 'sent', 'cancel')"
            instance.order_count = self.query_to_sale_order_count(order_query)
            risky_order_query = (
                sale_query + " and state IN ('draft') and is_risky_order = true"
            )
            instance.risk_order_count = self.query_to_sale_order_count(
                risky_order_query
            )
            # Below is used for picking count.
            picking_query = self.prepare_query_to_count_record(
                "stock_picking", instance
            )
            instance.picking_count = self.query_to_delivery_count(picking_query)
            instance.confirmed_picking_count = self.query_to_delivery_count(
                False, instance, "state", "confirmed"
            )
            instance.assigned_picking_count = self.query_to_delivery_count(
                False, instance, "state", "assigned"
            )
            instance.done_picking_count = self.query_to_delivery_count(
                False, instance, "state", "done"
            )
            ...
            open_invoice_query = (
                invoice_query
                + " and state='posted' and type='out_invoice' and invoice_payment_state != 'paid'"
            )
            instance.open_invoice_count = self.query_to_invoice_count(
                open_invoice_query
            )
            paid_invoice_query = (
                invoice_query
            ...
            instance.paid_invoice_count = self.query_to_invoice_count(
                paid_invoice_query
            )
            refund_invoice_query = invoice_query + " and type='out_refund'"
            instance.refund_invoice_count = self.query_to_invoice_count(
                refund_invoice_query
            )
    ...
    def _default_refund_adjustment_product(self):
        ...
        """
        refund_adjustment_product = self.env.ref(
    ...
    shopify_warehouse_id = fields.Many2one(
        "stock.warehouse", string="Warehouse", default=_get_default_warehouse
    )
    ...
    shopify_stock_field = fields.Many2one(
        "ir.model.fields", string="Stock Field", default=_default_stock_field
    )
    ...
    shopify_sync_product_with = fields.Selection(
        [
            ...
            ("barcode", "Barcode"),
            ("sku_or_barcode", "Internal Reference or Barcode"),
        ],
    ...
    update_category_in_odoo_product = fields.Boolean(
        string="Update Category In Odoo Product ?", default=False
    )
    ...
    is_use_default_sequence = fields.Boolean(
    ...
    shopify_store_time_zone = fields.Char(
        "Store Time Zone", help="This field used to import order process"
    )
    default_active_instance = fields.Boolean(
        string="Set as default active instance",
    ...
    gift_card_product_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref(
            "shopify_ept.shopify_gift_card_product", raise_if_not_found=False
        ),
    )
    ...
    # Auto cron field
    shopify_auto_cron = fields.Boolean(
        string="Shopify Auto Cron", help="Configure Auto cron"
    )
    ...
    invoice_tax_account_id = fields.Many2one(
        "account.account", string="Invoice Tax Account"
    )
    credit_tax_account_id = fields.Many2one(
    ...
    exported_product_count = fields.Integer(
        compute="_count_all", string="Exported Products"
    )

    ready_to_expor_product_count = fields.Integer(
        compute="_count_all", string="Ready For Export"
    )

    published_product_count = fields.Integer(
        compute="_count_all", string="Published Product"
    )

    unpublished_product_count = fields.Integer(
        compute="_count_all", string="#UnPublished Product"
    )
    ...
    confirmed_picking_count = fields.Integer(
        compute="_count_all", string="Confirm Picking"
    )

    assigned_picking_count = fields.Integer(
        compute="_count_all", string="Assigned Pickings"
    )
    ...
    refund_invoice_count = fields.Integer(
        compute="_count_all", string="Refund Invoices"
    )
    ...
    shopify_user_ids = fields.Many2many(
        "res.users",
        "shopify_instance_ept_res_users_rel",
        "res_config_settings_id",
        "res_users_id",
        string="Responsible User",
    )  # add by bhavesh jadav 28/11/2019 for set  responsible user for schedule activity
    shopify_activity_type_id = fields.Many2one(
        "mail.activity.type", string="Activity Type"
    )  # add by bhavesh jadav 28/11/2019 for set activity for schedule activity
    shopify_date_deadline = fields.Integer(
        "Deadline lead days",
        help="its add number of  days in schedule activity deadline date ",
    )  # add by bhavesh jadav 28/11/2019 for add date_deadline lead days user for schedule activity
    ...
    )  # add by bhavesh jadav 04/12/2019
    ...
    transaction_line_ids = fields.One2many(
        "shopify.payout.account.config.ept", "instance_id", string="Transaction Line"
    )
    ...
    def create(self, vals):
        ...
        """
        customer_vals = {
            "name": "POS Customer(%s)" % vals.get("name"),
            "customer_rank": 1,
        }
    ...
    def shopify_action_archive_unarchive(self):
        ...
        sale_auto_workflow_configuration_obj = self.env[
    ...
    def configure_webhooks(self, topic_list):
        ...
        if getattr(self, "create_shopify_%s_webhook" % (resource)):
            if available_webhooks:
                ...
                _logger.info(
                    "{} Webhooks are activated of instance '{}'.".format(
                        resource, self.name
                    )
                )
                topic_list = list(
                    set(topic_list) - set(available_webhooks.mapped("webhook_action"))
                )

            for topic in topic_list:
                webhook_obj.create(
                    {
                        "webhook_name": self.name + "_" + topic.replace("/", "_"),
                        "webhook_action": topic,
                        "instance_id": instance_id,
                    }
                )
                _logger.info(
                    "Webhook for '{}' of instance '{}' created.".format(
                        topic, self.name
                    )
                )
        else:
            if available_webhooks:
                ...
                _logger.info(
                    "{} Webhooks are paused of instance '{}'.".format(
                        resource, self.name
                    )
                )
    ...
    def refresh_webhooks(self):
        ...
        _logger.info(
            "Emipro-Webhook: Current webhook present in shopify is %s" % webhook_ids
        )
        ...
        if webhook_obj:
            for webhooks in webhook_obj:
                _logger.info(
                    "Emipro-Webhook: deleting the %s shopify.webhook.ept record"
                    % webhooks.id
                )
                self._cr.execute(
                    "DELETE FROM shopify_webhook_ept WHERE id = %s",
                    [webhooks.id],
    ...
    def action_archive(self):
        action = (
            ...
            .action_shopify_active_archive_instance()
            if self
            else True
        )
        return action
