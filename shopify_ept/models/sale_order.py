...
class SaleOrder(models.Model):
    ...
    shopify_instance_id = fields.Many2one(
        "shopify.instance.ept", "Instance", copy=False
    )
    shopify_order_status = fields.Char(
        "Shopify Order Status",
        copy=False,
        help="Shopify order status when order imported in odoo at the moment order status in Shopify.",
    )
    auto_workflow_process_id = fields.Many2one(
        "sale.workflow.process.ept", "Auto Workflow", copy=False
    )
    shopify_payment_gateway_id = fields.Many2one(
        "shopify.payment.gateway.ept", string="Payment Gateway", copy=False
    )
    risk_ids = fields.One2many(
        "shopify.order.risk", "odoo_order_id", "Risks", copy=False
    )
    shopify_location_id = fields.Many2one(
        "shopify.location.ept", "Shopify Location", copy=False
    )
    checkout_id = fields.Char("Checkout Id", copy=False)
    is_risky_order = fields.Boolean(
        "Risky Order ?", compute=_check_risk_order, store=True
    )
    updated_in_shopify = fields.Boolean(
        "Updated In Shopify ?",
        compute=_get_shopify_order_status,
        search="_search_shopify_order_ids",
    )
    closed_at_ept = fields.Datetime("Closed At", copy=False)
    canceled_in_shopify = fields.Boolean(
        "Canceled In Shopify", default=False, copy=False
    )
    is_service_tracking_updated = fields.Boolean(
        "Service Tracking Updated", default=False, copy=False
    )
    is_pos_order = fields.Boolean("POS Order ?", copy=False)
    is_shopify_multi_payment = fields.Boolean(
        "Multi Payments?",
        default=False,
        copy=False,
        help="It is used to identify that order has multi-payment gateway or not",
    )
    shopify_payment_ids = fields.One2many(
        "shopify.order.payment.ept", "order_id", string="Payment Lines"
    )

    _sql_constraints = [
        (
            "unique_shopify_order",
    ...
    def update_warehouse_shopify_order(self, shopify_location, warehouse_id, pos_order):
        return {
            "shopify_location_id": shopify_location and shopify_location.id or False,
    ...
    def import_shopify_orders(self, order_data_queue_line, log_book_id):
        ...
        _logger.info(
            "Start process of shopify order(%s) and order id is(%s) "
        ...
        if order_id:
            ...
            _logger.info(
                "Done the Process of order Because Shopify Order(%s) is exist in Odoo and "
                "Odoo order is(%s)"
                % (order_response.get("order_number"), order_id.name)
            )
            self._cr.commit()
            return order_id
        ...
        if order_response.get("source_name", "") == "pos":
            if order_response.get("customer"):
                partner = res_partner_obj.create_shopify_pos_customer(
                    order_response, log_book_id, instance
                )
            else:
            ...
            if not any(
                [
                    order_response.get("customer", False),
                    order_response.get("shipping_address", False),
                ...
                comman_log_line_obj.shopify_create_order_log_line(
                    message, model_id, order_data_queue_line, log_book_id
                )
                order_data_queue_line.write(
                    {"state": "failed", "processed_at": datetime.now()}
                )
                _logger.info(
                    "Customer not available in Shopify Order(%s)"
                    % (order_response.get("order_number"))
                )
                return False
            partner = (
                order_response.get("customer", {})
                and res_partner_obj.create_or_update_customer(
                    vals=order_response,
                    ...
                    type=False,
                    instance=instance,
                    order_data_queue_line=order_data_queue_line,
                )
                or False
            )
        ...
        shipping_address = (
            order_response.get("shipping_address", False)
            and res_partner_obj.create_or_update_customer(
                vals=order_response.get("shipping_address"),
                log_book_id=log_book_id,
                is_company=False,
                parent_id=parent_id.id if parent_id else False,
                type="delivery",
                instance=instance,
                email=order_response.get("customer", {}).get("email"),
            )
            or partner
        )
        # add by bhavesh jadav 05/12/2019
        invoice_address = (
            order_response.get("billing_address", False)
            and res_partner_obj.create_or_update_customer(
                vals=order_response.get("billing_address"),
                log_book_id=log_book_id,
                is_company=False,
                parent_id=parent_id.id if parent_id else False,
                type="invoice",
        ...
        if self.check_mismatch_details(
            lines,
            instance,
            order_response.get("order_number"),
            ...
            _logger.info(
                "Mis-mismatch details found in this Shopify Order(%s) and id (%s)"
                % (order_response.get("order_number"), order_response.get("id"))
            )
            order_data_queue_line.write(
                {"state": "failed", "processed_at": datetime.now()}
        ...
        order_id = self.shopify_create_order(
            instance,
            partner,
            shipping_address,
            invoice_address,
        ...
        if not order_id:
            _logger.info(
                "Configuration missing in Odoo while importing Shopify Order(%s) "
                "and id (%s)"
                % (order_response.get("order_number"), order_response.get("id"))
            )
            order_data_queue_line.write(
        ...
        order_id.write(
            order_id.update_warehouse_shopify_order(
                shopify_location, warehouse_id, pos_order
            )
        )

        risk_result = shopify.OrderRisk().find(order_id=order_response.get("id"))
        if risk_result:
            self.env["shopify.order.risk"].shopify_create_risk_in_order(
                risk_result, order_id
            )
        _logger.info(
            "Creating order line for Odoo order(%s) and Shopify order is (%s)"
        ...
        for line in lines:
            ...
            order_line = self.shopify_create_sale_order_line(
                line,
                product,
                line.get("quantity"),
                product.name,
            ...
            if float(total_discount) > 0.0:
                self.create_discount_allocation_details_line(
                    line, order_id, order_response, instance, product, order_line
                )
        _logger.info(
            "Created order line for Odoo order(%s) and Shopify order is (%s)"
            % (order_id.name, order_response.get("order_number"))
        )
        ...
        if order_response.get("fulfillment_status") == "fulfilled":
            order_id.auto_workflow_process_id.shipped_order_workflow(
                order_id, customer_loc
            )
            # Below code add for create partially/fully refund
            message = self.create_shipped_order_refund(
                shopify_financial_status, order_response, order_id
            )
        else:
        ...
        _logger.info(
            "Done auto workflow process for Odoo order(%s) and Shopify order is (%s)"
            % (order_id.name, order_response.get("order_number"))
        )
        if message and order_data_queue_line:
            model_id = comman_log_line_obj.get_model_id(self._name)
            comman_log_line_obj.shopify_create_order_log_line(
                message, model_id, order_data_queue_line, log_book_id
            )
            order_data_queue_line.write(
        ...
        _logger.info(
            "Done the Odoo Order(%s) create process and Shopify Order (%s)"
            % (order_id.name, order_response.get("order_number"))
        )
        return order_id

    def create_shipped_order_refund(
        self, shopify_financial_status, order_response, sale_order
    ):
        """This method is used to create partially or fully refund in shopify order.
        @param : shopify_financial_status, order_response, sale_order
        @return: message
        @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 30 December 2021 .
        Task_id: 181893
        """
        message = ""
        if shopify_financial_status == "refunded":
            ...
            for refund in order_response.get("refunds"):
                ...
                if (
                    ...
                    and refund.get("transactions")[0].get("kind") == "refund"
                    and refund.get("transactions")[0].get("status") == "success"
                ):
                    refunded_amount = refund.get("transactions")[0].get("amount")
                    total_refund += float(refunded_amount)
            refunded = sale_order.create_shopify_refund(
                order_response.get("refunds"), total_refund
            )
            if refunded[0] == 0:
                message = (
                    "- Refund can only be generated if it's related order "
                ...
                message = (
                    "- Partial refund is received from Shopify for order [%s].\n "
        ...
        elif shopify_financial_status == "partially_refunded" and order_response.get(
            "refunds"
        ):
            message = sale_order.create_shopify_partially_refund(
                order_response.get("refunds"), order_response.get("name")
            )
    ...
    def check_and_create_stock_move_based_on_mrp(self, order_id):
        ...
        is_mrp_install = (
            ...
            .sudo()
            .search([("name", "=", "mrp"), ("state", "=", "installed")])
        )
        picking_obj = self.env["stock.picking"]

        for line in order_id.order_line.filtered(
            lambda l: l.product_id.type != "service"
            and l.shopify_fulfillable_quantity > 0
        ):
            ...
            if is_mrp_install and bom_lines:
                for bom_line in bom_lines:
                    self.shopify_create_and_done_stock_move_ept(
                        line, order_id, bom_line
                    )
            else:
    ...
    def shopify_create_and_done_stock_move_ept(self, line, order_id, bom_line=False):
        ...
        product_uom_qty = (
            bom_line[1].get("qty", 0) * product_qty if bom_line else product_qty
        )
    ...
    def create_discount_allocation_details_line(
        self, line, order_id, order_response, instance, product, order_line
    ):
    ...
    def create_or_update_shipping_lines_ept(self, instance, order_id, order_response):
        ...
        for line in order_response.get("shipping_lines", []):
            carrier = self.env[
                "delivery.carrier"
            ].shopify_search_create_delivery_carrier(line, instance)
        ...
        _logger.info(
            "Start auto workflow process for Odoo order(%s) and Shopify order is (%s)"
            % (order_id.name, order_response.get("order_number"))
        )
        return True

    def check_mismatch_details(
        self, lines, instance, order_number, order_data_queue_line, log_book_id
    ):
        ...
        for line in lines:
            if (
                line.get("fulfillment_service") == "gift_card"
                or line.get("name") == "Tip"
            ):
                continue
            ...
            if not shopify_variant:
                ...
                if line_product_id and line_variant_id:
                    ...
                    message = "Product Id Not Available In %s Order Line response" % (
                        order_number
                    )
                    comman_log_line_obj.shopify_create_order_log_line(
                        message, model_id, order_data_queue_line, log_book_id
    ...
    def shopify_create_order(
        ...
        (
            payment_gateway,
            workflow,
        ) = payment_gateway_obj.shopify_search_create_gateway_workflow(
            instance, order_data_queue_line, order_response, log_book_id, gateway
        )

        if not all([payment_gateway, workflow]):
            return False

        order_vals = self.prepare_shopify_order_vals(
            instance,
            partner,
            shipping_address,
            invoice_address,
        ...
        if len(order_response.get("payment_gateway_names")) > 1:
            payment_vals = self.prepare_vals_shopify_multi_payment(
                instance,
                order_data_queue_line,
                order_response,
                log_book_id,
            ...
            if not payment_vals:
                return False
            order_vals.update(
                {"shopify_payment_ids": payment_vals, "is_shopify_multi_payment": True}
            )
    ...
    def prepare_shopify_order_vals(
        ...
        pricelist_id = self.shopify_set_pricelist(
            order_response=order_response, instance=instance
        )
        ordervals = {
            "company_id": instance.shopify_company_id.id
            if instance.shopify_company_id
            else False,
            "partner_id": partner.ids[0],
            "partner_invoice_id": invoice_address.ids[0],
            "partner_shipping_id": shipping_address.ids[0],
            "warehouse_id": instance.shopify_warehouse_id.id
            if instance.shopify_warehouse_id
            else False,
            "date_order": date_order,
            "state": "draft",
            "pricelist_id": pricelist_id.id if pricelist_id else False,
            "team_id": instance.shopify_section_id.id
            if instance.shopify_section_id
            else False,
        }
        ordervals = self.create_sales_order_vals_ept(ordervals)
        order_response_vals = self.prepare_order_vals_from_order_response(
    ...
    def convert_order_date(self, order_response):
        ...
        if order_response.get("created_at", False):
            ...
            date_order = (
                parser.parse(order_date).astimezone(utc).strftime("%Y-%m-%d %H:%M:%S")
            )
        else:
    ...
    def prepare_order_vals_from_order_response(
        ...
        order_vals = {
            ...
            "shopify_order_id": order_response.get("id"),
            "shopify_order_number": order_response.get("order_number"),
            "shopify_payment_gateway_id": payment_gateway
            and payment_gateway.id
            or False,
            "shopify_instance_id": instance.id,
    ...
    def shopify_create_sale_order_line(
        ...
        if order_id.shopify_instance_id.apply_tax_in_order == "create_shopify_tax":
            ...
            if line and line.get("tax_lines"):
                if line.get("taxable"):
                    ...
                    # taxable
                    tax_ids = self.shopify_get_tax_id_ept(
                        order_id.shopify_instance_id,
                        line.get("tax_lines"),
                        taxes_included,
                    )
                if is_shipping:
        ...
        if is_discount:
            ...
            if (
                order_id.shopify_instance_id.apply_tax_in_order == "odoo_tax"
                and is_discount
            ):  # add by bhavesh jadav 04/12/2019 for by pass odoo tax flow on discount line
                ...
                acctax_id = self.env["account.tax"].search(
                    [
                        ...
                        (
                            "company_id",
                            "=",
                            instance.shopify_warehouse_id.company_id.id,
                        ),
                    ],
                    limit=1,
                )
                if not acctax_id:
                    acctax_id = self.shopify_create_account_tax(
                        instance, rate, tax_included, company, name
                    )
                if acctax_id:
    ...
    @api.model
    def shopify_create_account_tax(
        self, instance, value, price_included, company, name
    ):
        ...
        account_tax_id.mapped("invoice_repartition_line_ids").write(
            {
                "account_id": instance.invoice_tax_account_id.id
                if instance.invoice_tax_account_id
                else False
            }
        )
        account_tax_id.mapped("refund_repartition_line_ids").write(
            {
                "account_id": instance.credit_tax_account_id.id
                if instance.credit_tax_account_id
                else False
            }
        )
    ...
    def prepare_vals_shopify_multi_payment(
        self,
        instance,
        order_data_queue_line,
        order_response,
        ...
        for transaction in transactions:
            ...
            if (
                result.get("kind") in ["capture", "sale"]
    ...
    def update_order_status_in_shopify(self, instance):
        ...
        model_id = comman_log_line_obj.get_model_id(model)
        _logger.info(
            _("Update Order Status process start and you select '%s' Instance")
            % instance.name
        )
        ...
        for picking_id in picking_ids:
            ...
            if order_lines and order_lines.filtered(
                ...
                and s.shopify_line_id == False
                or ""
            ):
                message = _(
                    "Order status is not updated for order %s because shopify line id not found in this order."
                    % picking.sale_id.name
                )
                _logger.info(message)
                log_line_array = shopify_product_obj.shopify_create_log(
                    message, model_id, False, log_line_array
                )
                continue
            ...
            if not manage_multi_tracking_number_in_shopify_delivery_order:
                not_kit_moves = picking.move_lines.filtered(
                    lambda x: x.sale_line_id.product_id.id == x.product_id.id
                    and x.state == "done"
                )
                ...
                for move in not_kit_moves:
                    ...
                    for move_line in move.move_line_ids:
                        ...
                        tracking_number_wise_details.update(
                            {
                                move_line.result_package_id.tracking_no
                                or "No_Tracking": (qty + move_line.qty_done)
                            }
                        )
                ...
                for move in kit_move_lines:
                    ...
                    if move.sale_line_id and move.sale_line_id.shopify_line_id:
                        shopify_line_id = move.sale_line_id.shopify_line_id
                    tracking_no = (
                        move.move_line_ids.mapped("result_package_id").mapped(
                            "tracking_no"
                        )
                        or []
                    )
            ...
            if not shopify_location_id:
                ...
                if not shopify_location_id:
                    message = (
                        "Primary Location not found for instance %s while Update order status"
                        % (instance.name)
                    )
                    _logger.info(message)
                    log_line_array = shopify_product_obj.shopify_create_log(
                    ...
                    new_fulfillment = shopify.Fulfillment(
                        {
                            ...
                            "location_id": shopify_location_id.shopify_location_id,
                            "tracking_numbers": list(set(tracking_numbers)),
                            "tracking_urls": self.get_shopify_tracking_number_url(
                                picking
                            ),
                            "tracking_company": carrier_name,
                            "line_items": update_lines,
            ...
            try:
                ...
                message = (
                    "Order(%s) status not updated due to some issue in fulfillment "
                    "request/response: %s" % (sale_order.name, str(e))
                )
                _logger.info(message)
                log_line_array = shopify_product_obj.shopify_create_log(
                    message, model_id, False, log_line_array
            ...
            if not fulfillment_result:
                ...
                message = (
                    "Order(%s) status not updated due to some issue in fulfillment request/response:"
                    % (sale_order.name)
                )
                _logger.info(message)
                log_line_array = shopify_product_obj.shopify_create_log(
            ...
            if new_fulfillment:
                ...
                if shopify_fullment_result:
                    fulfillment_id = (
                        shopify_fullment_result.get("fulfillment").get("id") or ""
                    )
            picking.write(
                {"updated_in_shopify": True, "shopify_fulfillment_id": fulfillment_id}
            )
    ...
    def process_shopify_order_via_webhook(
        self, order_data, instance, update_order=False
    ):
        """
        ...
        order_data_queue = self.env[
            "shopify.order.data.queue.line.ept"
        ].create_order_data_queue_line(
            [order_data],
            instance,
            created_by="webhook",
        ...
        self._cr.commit()
        order_data_queue = self.env["shopify.order.data.queue.ept"].browse(
            order_data_queue
        )
        if order_data_queue:
            if not update_order:
                ...
                _logger.info(
                    "Imported order {} of {} via Webhook Successfully".format(
                        order_data.get("id"), instance.name
                    )
                )
        _logger.info(
            "Processed order {} of {} via Webhook Successfully".format(
                order_data.get("id"), instance.name
            )
        )
        return True
    ...
    def update_shopify_order(self, queue_line, log_book):
        ...
        if order_data.get("cancel_reason"):
            ...
            if not cancelled:
                message = "System can not cancel the order {} as one of the picking is in the done state.".format(
                    order.name
                )
        if shopify_status == "refunded":
            if not message:
                ...
                for refund in order_data.get("refunds"):
                    ...
                    if (
                        ...
                        refunded_amount = refund.get("transactions")[0].get("amount")
                        total_refund += float(refunded_amount)
                refunded = order.create_shopify_refund(
                    order_data.get("refunds"), total_refund
                )
                if refunded[0] == 0:
                    message = (
                        "System can not generate a refund as the invoice is not found. "
                    )
            ...
            message = order.create_shopify_partially_refund(
                order_data.get("refunds"), order_data.get("name")
            )
        # Below condition use for, In shopify store there is fulfilled order.
            ...
            if isinstance(fulfilled, bool) and not fulfilled:
                ...
                message = "System can not complete the picking as {}".format(fulfilled)
        ...
        if message:
            ...
            model_id = comman_log_line_obj.get_model_id(model)
            comman_log_line_obj.shopify_create_order_log_line(
                message, model_id, queue_line, log_book
            )
            ...
            move_reversal = self.env["account.move.reversal"].create(
                {
                    ...
                    else refunds_data[0].get("note"),
                }
            )
            move_reversal.with_context(
    ...
    def fulfilled_shopify_order(self):
        ...
        if self.state not in ["sale", "done", "cancel"]:
            self.action_confirm()
        return self.fulfilled_picking_for_shopify(
        ...
        for picking in pickings.filtered(lambda x: x.state not in ["cancel", "done"]):
            if picking.state != "assigned":
                ...
                if picking.sale_id and (
                    picking.sale_id.is_pos_order
                    or picking.sale_id.shopify_order_status == "fulfilled"
                ):
    ...
    def create_shopify_partially_refund(self, refunds_data, order_name):
        ...
        if not self.invoice_ids:
            message = (
                "- Partially refund can only be generated if it's related order "
                "invoice is found.\n- For order [%s], system could not find the "
        ...
        for refund_data_line in refunds_data:
            existing_refund = account_move_obj.search(
                [
                    ("shopify_refund_id", "=", refund_data_line.get("id")),
                    ("shopify_instance_id", "=", self.shopify_instance_id.id),
                ]
            )
            if existing_refund:
                continue
            new_move = self.create_move_and_delete_not_necessary_line(
                refund_data_line, invoices
            )
            if refund_data_line.get("order_adjustments"):
                self.create_refund_adjustment_line(
                    refund_data_line.get("order_adjustments"), new_move
                )
    ...
    def create_move_and_delete_not_necessary_line(self, refunds_data, invoices):
        ...
        for refund_line in refunds_data.get("refund_line_items"):
            ...
            shopify_line_ids_with_qty.update(
                {refund_line.get("line_item_id"): refund_line.get("quantity")}
            )
            ...
            .create(
                {
                    ...
                    "reason": "Partially Refunded from shopify"
                    if len(refunds_data) > 1
                    else refunds_data.get("note"),
                }
            )
        )
        ...
        new_move.write(
            {"is_refund_in_shopify": True, "shopify_refund_id": refunds_data.get("id")}
        )
        for new_move_line in new_move.invoice_line_ids:
            shopify_line_id = new_move_line.sale_line_ids.shopify_line_id
            if (
                shopify_line_id
                and int(shopify_line_id) not in shopify_line_ids
                or new_move_line.product_id
                == self.shopify_instance_id.discount_product_id
            ):
                delete_move_lines += new_move_line
            else:
                new_move_line.with_context(
                    check_move_validity=False
                ).quantity = shopify_line_ids_with_qty.get(int(shopify_line_id))

        new_move.message_post(
            body=_(
                "Credit note generated by Webhook as Order partially "
        ...
        self.message_post(
            body=_(
                "Partially credit note created <a href=# data-oe-model=account.move data-oe-id=%d>%s</a> via webhook"
            )
            % (new_move.id, new_move.name)
        )
    ...
    def create_refund_adjustment_line(self, order_adjustments, move_ids):
        ...
        adjustment_product = self.env.ref(
        ...
        if abs(adjustments_amount) > 0:
            move_vals = {
                ...
                "quantity": 1,
                "price_unit": abs(adjustments_amount),
                "move_id": move_ids.id,
                "partner_id": move_ids.partner_id.id,
            ...
            new_vals.update(
                {"quantity": 1, "price_unit": abs(adjustments_amount), "tax_ids": []}
            )
    ...
    def cancel_in_shopify(self):
        ...
        context.update(
            {"active_model": "sale.order", "active_id": self.id, "active_ids": self.ids}
        )
        return {
    ...
    def paid_invoice_ept(self, invoices):
        ...
        Override the auto invoice workflow library method here to create separate payment records.
        """
        self.ensure_one()
