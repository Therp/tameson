...
class ShopifyWebhookEpt(models.Model):
    ...
    instance_id = fields.Many2one(
        ...
        string="Webhook created by this Shopify Instance.",
        ondelete="cascade",
    )
            ...
            if record.webhook_id:
                ...
                try:
                    ...
                    if address[: address.find(route)] == url:
                        webhook.destroy()
                        _logger.info(
                            "Deleted %s webhook from Store." % record.webhook_action
                        )
                except:
    ...
    def deactivate_auto_create_webhook(self, instance):
        ...
        all_webhook_action = self.search([("instance_id", "=", instance.id)]).mapped(
            "webhook_action"
        )
        if instance.create_shopify_products_webhook:
            ...
            if not result:
                ...
                _logger.info(
                    "Inactive create_shopify_products_webhook from the %s instance"
                    % instance.name
                )
        if instance.create_shopify_customers_webhook:
            ...
            if not result:
                ...
                _logger.info(
                    "Inactive create_shopify_customers_webhook from the %s instance"
                    % instance.name
                )
        if instance.create_shopify_orders_webhook:
            ...
            if not result:
                ...
                _logger.info(
                    "Inactive create_shopify_orders_webhook from the %s instance"
                    % instance.name
                )
    ...
    def get_webhook(self):
        ...
        if url[: url.find(":")] == "http":
            raise Warning(
                "Address protocol http:// is not supported while creating the webhook"
            )
