...
class ShopifyProcessImportExport(models.TransientModel):
    ...
    shopify_operation = fields.Selection(
        [
            ("sync_product", "Sync New Products - Set To Queue"),
            ("sync_product_by_remote_ids", "Sync New Products - By Remote Ids"),
    ...
    shopify_is_set_price = fields.Boolean(
        string="Set Price ?",
        help="If is a mark, it set the price with product in the Shopify store.",
        default=False,
    )
    shopify_is_set_stock = fields.Boolean(
        string="Set Stock ?",
    ...
    shopify_is_set_image = fields.Boolean(
        string="Set Image ?",
        help="If is a mark, it set the image with product in the Shopify store.",
        default=False,
    )
    shopify_is_set_basic_detail = fields.Boolean(
        string="Set Basic Detail ?",
        help="If is a mark, it set the product basic detail in shopify store",
        default=True,
    )
    shopify_is_update_basic_detail = fields.Boolean(
        string="Update Basic Detail ?",
    ...
    shopify_template_ids = fields.Text(
        string="Template Ids",
        help="Based on template ids get product from shopify and import products in odoo",
    )
    shopify_order_ids = fields.Text(
        string="Order Ids",
        help="Based on template ids get product from shopify and import products in odoo",
    )
    export_stock_from = fields.Datetime(
        help="It is used for exporting stock from Odoo to Shopify."
    )
    ...
    def shopify_execute(self):
        ...
        if self.shopify_operation == "sync_product":
            product_queues = product_data_queue_obj.shopify_create_product_data_queue(
                instance
            )
            if product_queues:
                action = self.env.ref(
                    "shopify_ept.action_shopify_product_data_queue"
                ).read()[0]
                action["domain"] = [("id", "in", product_queues)]
                return action
        if self.shopify_operation == "sync_product_by_remote_ids":
            product_queues = product_data_queue_obj.shopify_create_product_data_queue(
                instance, self.shopify_template_ids
            )
            if product_queues:
                ...
                _logger.info(
                    "Processed product queue : {} of Instance : {} Via Product Template ids Suuceessfully .".format(
        ...
        if self.shopify_operation == "import_customers":
            ...
            if customer_queues:
                action = self.env.ref(
                    "shopify_ept.action_shopify_synced_customer_data"
                ).read()[0]
                action["domain"] = [("id", "in", customer_queues)]
                return action
        if self.shopify_operation == "import_orders":
            order_queues = order_date_queue_obj.shopify_create_order_data_queues(
                instance, self.orders_from_date, self.orders_to_date
            )
            if order_queues:
                action = self.env.ref(
                    "shopify_ept.action_shopify_order_data_queue_ept"
                ).read()[0]
                action["domain"] = [("id", "in", order_queues)]
                return action
        if self.shopify_operation == "import_orders_by_remote_ids":
            order_date_queue_obj.import_order_process_by_remote_ids(
                instance, self.shopify_order_ids
            )
        ...
        if self.shopify_operation == "import_payout_report":
            if self.payout_end_date and self.payout_start_date:
                ...
                self.env["shopify.payout.report.ept"].get_payout_report(
                    self.payout_start_date, self.payout_end_date, instance
                )
        return {
    ...
    def manual_export_product_to_shopify(self):
        ...
        if templates:
            shopify_product_obj.shopify_export_products(
                templates.shopify_instance_id,
    ...
    def manual_update_product_to_shopify(self):
        if (
            not self.shopify_is_update_basic_detail
            and not self.shopify_is_publish
            and not self.shopify_is_set_price
        ...
        if templates:
            shopify_product_obj.update_products_in_shopify(
                templates.shopify_instance_id,
                self.shopify_is_set_price,
                self.shopify_is_set_image,
                self.shopify_is_publish,
        ...
        if len(customer_ids) > 0:
            ...
            if len(customer_ids) > 150:
                for customer_id_chunk in split_every(150, customer_ids):
                    customer_queue_id = data_queue.shopify_create_customer_queue(
                        self.shopify_instance_id, "import_process"
                    )
                    customer_queue = self.shopify_create_multi_queue(
                        customer_queue_id, customer_id_chunk
                    )
                ...
                customer_queue = self.shopify_create_multi_queue(
                    customer_queue_id, customer_ids
                )
                customer_queue_list.append(customer_queue.id)
    ...
    def shopify_customer_data_queue_line_create(self, result, customer_queue_id):
        ...
        """
        synced_shopify_customers_line_obj = self.env[
    ...
    def webhook_customer_create_process(self, res, instance):
        ...
        customer_queue_id = data_queue.search(
            [
                ("record_created_from", "=", "webhook"),
                ("state", "=", "draft"),
                ("shopify_instance_id", "=", instance.id),
            ],
            limit=1,
        ...
        if customer_queue_id:
            _logger.info(
                "Customer %s added into Queue %s."
                % (res.get("first_name"), customer_queue_id.name)
            )
        else:
            customer_queue_id = data_queue.shopify_create_customer_queue(
                instance, "webhook"
        ...
        _logger.info(
            "process end : shopify odoo webhook for customer route call and customer"
            "queue is %s" % customer_queue_id.name
        )
                ...
                if page_link.find("next") > 0:
                    ...
                    try:
                        ...
                        if (
                            ...
                            result = shopify.Customer().find(
                                page_info=page_info, limit=200
                            )
                        else:
                            raise Warning(e)
            if catch == page_info:
    ...
    def update_stock_in_shopify(self, ctx={}):
        ...
        if self.export_stock_from:
            ...
            _logger.info(
                "Exporting Stock from Operations wizard for instance - %s....."
                % instance.name
            )
        else:
            last_update_date = (
                instance.shopify_last_date_update_stock
                or datetime.now() - timedelta(30)
            )
            _logger.info(
        ...
        if products:
            ...
            if shopify_products:
                instance.write(
                    {
                        "shopify_last_date_update_stock": shopify_products[
                            0
    ...
    def shopify_selective_product_stock_export(self):
        ...
        for instance_id in shopify_instance_ids:
            product_id = (
                self.env["shopify.product.product.ept"]
                .search(
                    [
                        ("shopify_instance_id", "=", instance_id.id),
                        ("shopify_template_id", "in", shopify_product_template_ids),
                    ]
                )
                .product_id.ids
            )
            if product_id:
                self.env["shopify.product.product.ept"].with_context(
                    {"is_process_from_selected_product": True}
                ).export_stock_in_shopify(instance_id, product_id)
    ...
    def update_order_status_cron_action(self, ctx={}):
        ...
        instance = self.env["shopify.instance.ept"].browse(instance_id)
        _logger.info(
            _("Auto cron update order status process start with instance: '%s'")
            % instance.name
        )
