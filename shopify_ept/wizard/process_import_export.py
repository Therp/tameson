...
class ShopifyProcessImportExport(models.TransientModel):
        ...
        if len(customer_ids) > 0:
            ...
            if len(customer_ids) > 150:
                for customer_id_chunk in split_every(150, customer_ids):
                    customer_queue_id = data_queue.shopify_create_customer_queue(self.shopify_instance_id,
                                                                                 "import_process")
    ...
    def webhook_customer_create_process(self, res, instance):
        ...
        customer_queue_id = data_queue.search([
            ("record_created_from", "=", "webhook"), ("state", "=", "draft"),
            ("shopify_instance_id", "=", instance.id)], limit=1)

        if customer_queue_id:
            _logger.info("Customer %s added into Queue %s." % (res.get("first_name"), customer_queue_id.name))
        else:
    ...
    def update_stock_in_shopify(self, ctx={}):
        ...
        if products:
            ...
            shopify_products = shopify_product_obj.export_stock_in_shopify(instance, product_id_array)
            if shopify_products:
        ...
        for instance_id in shopify_instance_ids:
            ...
            if product_id:
                self.env['shopify.product.product.ept'].with_context({
                    'is_process_from_selected_product': True}).export_stock_in_shopify(instance_id, product_id)
