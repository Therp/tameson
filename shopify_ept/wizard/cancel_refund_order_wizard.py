...
class ShopifyCancelRefundOrderwizard(models.TransientModel):
    ...
    message = fields.Selection(
        ...
        string="Message",
        default="other",
        help="The reason for the " "order cancellation",
    )
    ...
    notify_by_email = fields.Boolean(
        "Notify By Email ?",
        help="Whether to send a refund " "notification to the customer.",
    )
    ...
    payment_ids = fields.Many2many(
        "shopify.order.payment.ept", "shopify_payment_refund_rel", string="Payments"
    )
    ...
    def shopify_create_credit_note(self, order_id):
        ...
        moves = order_id.invoice_ids.filtered(
            lambda m: m.type == "out_invoice" and m.invoice_payment_state == "paid"
        )
        if not moves:
            ...
            order_id._cr.commit()
            warning_message = (
                "Order cancel in Shopify But unable to create a credit note in Odoo \n "
    ...
    def refund_in_shopify(self):
        ...
        for credit_note_id in credit_note_ids:
                ...
                if restock_type in ["cancel", "return"]:
                    ...
                    if not shopify_location_id:
                        if restock_type == "cancel":
                            log_message = (
                                "Primary Location not found for instance %s while refund order (%s) in "
                                "shopify." % (shopify_instance.name, order_id.name)
                            )
                        else:
                            log_message = (
                                "Location is not set in order (%s).Unable to refund in "
                        ...
                        ].shopify_create_product_log_line(
            ...
            log_lines = self.create_refund_in_shopify(
                orders,
                credit_note_id,
                refund_lines_list,
                model_id,
            ...
            self.env["common.log.book.ept"].create(
                {
                    "type": "export",
                    "module": "shopify_ept",
                    "shopify_instance_id": credit_note_id.shopify_instance_id.id
                    if credit_note_id.shopify_instance_id
                    else False,
                    "active": True,
    ...
    def check_validation_of_multi_payment(self):
        ...
        if self.payment_ids and not (any(self.payment_ids.mapped("is_want_to_refund"))):
            raise UserError(
                _("Please select which payment gateway do you want a refund.")
            )
        return True

    def create_refund_in_shopify(
        self,
        orders,
        credit_note_id,
        refund_lines_list,
        ...
        for order in orders:
            ...
            vals = self.prepare_shopify_refund_vals(
                parent_id, gateway, refund_amount, order, shipping, refund_lines_list
            )
            ...
            try:
                ...
                log_message = (
                    "When creating refund in Shopify for order (%s), issue arrive in "
                    "request (%s)" % (order.name, error)
                )
                log_line = self.env[
                    "common.log.lines.ept"
                ].shopify_create_order_log_line(log_message, model_id, False, False)
    ...
    def refund_amount_validation(self, order, refund_amount):
        ...
        for transaction in transactions:
            ...
            if result.get("kind") == "refund" and result.get("status") == "success":
                ...
                total_refund_in_shopify = total_refund_in_shopify + float(
                    refunded_amount
                )
    ...
    def prepare_shopify_refund_vals(
        self, parent_id, gateway, refund_amount, order, shipping, refund_lines_list
    ):
        ...
        if multi_payments:
            ...
            for order_payment in multi_payments.filtered(
                ...
                transaction = self.preapre_refund_transaction_vals(
                    parent_id,
                ...
                remaining_refund_amount = (
                    order_payment.remaining_refund_amount - order_payment.refund_amount
                )
            ...
            transactions = self.preapre_refund_transaction_vals(
                parent_id, refund_amount, gateway
            )
