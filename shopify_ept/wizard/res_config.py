...
class ShopifyInstanceConfig(models.TransientModel):
    ...
    def shopify_test_connection(self):
        ...
        # Added by Harsh Parekh on 11/10/2020 when instance is create that time automatic create or update shipping product in the instance. Task Id:166790
        product_template = self.env["product.template"].search(
            [("name", "=", "shopifyshippingproduct"), ("type", "=", "service")], limit=1
        )
        if not product_template:
            product_template = self.env["product.template"].create(
                {"name": "shopifyshippingproduct", "type": "service"}
            )

        shopify_instance.write(
            {
                "shopify_global_channel_id": shopify_global_channel_id.id or False,
                "shopify_shipping_product_id": product_template.product_variant_ids[
                    0
                ].id,
            }
        )
...
class ResConfigSettings(models.TransientModel):
    ...
    import_shopify_order_status_ids = fields.Many2many(
    ...
    shopify_is_use_default_sequence = fields.Boolean(
    ...
    shopify_order_prefix = fields.Char(
        size=10, string="Order Prefix", help="Enter your order prefix"
    )
    shopify_discount_product_id = fields.Many2one(
        "product.product", "Discount", domain=[("type", "=", "service")], required=False
    )
    # Shipping Product
    shopify_shipping_product_id = fields.Many2one(
        "product.product",
        "Shipping Prpduct",
        domain=[("type", "=", "service")],
        required=False,
    )
    shopify_gift_card_product_id = fields.Many2one(
        "product.product", "Gift Card", domain=[("type", "=", "service")]
    )
    ...
    shopify_apply_tax_in_order = fields.Selection(
        ...
        copy=False,
        default="create_shopify_tax",
        help=""" For Shopify Orders :- \n
                    1) Odoo Default Tax Behaviour - The Taxes will be set based on Odoo's
                                 default functional behaviour i.e. based on Odoo's Tax and Fiscal Position configurations. \n
                    2) Create New Tax If Not Found - System will search the tax data received
                    from Shopify in Odoo, will create a new one if it fails in finding it.""",
    )
    shopify_invoice_tax_account_id = fields.Many2one(
        "account.account", string="Invoice Tax " "Account"
    )
    shopify_credit_tax_account_id = fields.Many2one(
        "account.account", string="Credit Tax Account"
    )
    ...
    shopify_notify_customer = fields.Boolean(
        "Notify Customer about Update Order Status?",
    ...
    shopify_global_channel_id = fields.Many2one(
        "global.channel.ept", string="Global Channel"
    )

    shopify_user_ids = fields.Many2many(
        "res.users",
        "shopify_res_config_settings_res_users_rel",
        "res_config_settings_id",
        "res_users_id",
        string="Responsible User",
    )  # add by bhavesh jadav
    shopify_activity_type_id = fields.Many2one(
    ...
    def onchange_shopify_instance_id(self):
        ...
        if instance:
            self.shopify_company_id = (
                instance.shopify_company_id and instance.shopify_company_id.id or False
            )
            self.shopify_warehouse_id = (
                instance.shopify_warehouse_id
                and instance.shopify_warehouse_id.id
                or False
            )
            self.shopify_country_id = (
                instance.shopify_country_id and instance.shopify_country_id.id or False
            )
            ...
            self.shopify_pricelist_id = (
                instance.shopify_pricelist_id
                and instance.shopify_pricelist_id.id
                or False
            )
            self.shopify_stock_field = (
                instance.shopify_stock_field
                and instance.shopify_stock_field.id
                or False
            )
            ...
            self.import_shopify_order_status_ids = (
                instance.import_shopify_order_status_ids.ids
            )
            ...
            self.shopify_discount_product_id = (
                instance.discount_product_id
                and instance.discount_product_id.id
                or False
            )
            self.shopify_shipping_product_id = (
                instance.shopify_shipping_product_id
                and instance.shopify_shipping_product_id
                or False
            )
            self.shopify_gift_card_product_id = (
                instance.gift_card_product_id
                and instance.gift_card_product_id.id
                or False
            )
            self.shopify_apply_tax_in_order = instance.apply_tax_in_order
            self.shopify_invoice_tax_account_id = (
                instance.invoice_tax_account_id
                and instance.invoice_tax_account_id.id
                or False
            )
            self.shopify_credit_tax_account_id = (
                instance.credit_tax_account_id
                and instance.credit_tax_account_id.id
                or False
            )
            ...
            self.shopify_global_channel_id = (
                instance and instance.shopify_global_channel_id or False
            )
            self.shopify_user_ids = (
                instance.shopify_user_ids or False
            )  # add by bhavesh jadav 28/11/2019
            self.shopify_activity_type_id = (
                instance.shopify_activity_type_id or False
            )  # add by bhavesh jadav 28/11/2019
            self.shopify_date_deadline = (
                instance.shopify_date_deadline or False
            )  # add by bhavesh jadav 28/11/2019
            self.is_shopify_create_schedule = (
                instance.is_shopify_create_schedule or False
            )
            self.shopify_sync_product_with_images = (
                instance.sync_product_with_images or False
            )
            self.create_shopify_products_webhook = (
                instance.create_shopify_products_webhook
            )
            self.create_shopify_customers_webhook = (
                instance.create_shopify_customers_webhook
            )
            self.create_shopify_orders_webhook = instance.create_shopify_orders_webhook

            self.shopify_default_pos_customer_id = (
                instance.shopify_default_pos_customer_id
            )
            self.last_date_order_import = instance.last_date_order_import or False
            self.shopify_last_date_customer_import = (
                instance.shopify_last_date_customer_import or False
            )
            self.shopify_last_date_update_stock = (
                instance.shopify_last_date_update_stock or False
            )
            self.shopify_last_date_product_import = (
                instance.shopify_last_date_product_import or False
            )
            self.shopify_payout_last_date_import = (
                instance.payout_last_import_date or False
            )
            self.shopify_settlement_report_journal_id = (
                instance.shopify_settlement_report_journal_id or False
            )
            self.tip_product_id = (
                instance.tip_product_id.id if instance.tip_product_id else False
            )
    ...
    def execute(self):
        ...
        if instance:
            ...
            values["shopify_fiscal_position_id"] = (
                self.shopify_fiscal_position_id
                and self.shopify_fiscal_position_id.id
                or False
            )
            values["shopify_company_id"] = (
                self.shopify_company_id and self.shopify_company_id.id or False
            )
            values["shopify_warehouse_id"] = (
                self.shopify_warehouse_id and self.shopify_warehouse_id.id or False
            )
            values["shopify_country_id"] = (
                self.shopify_country_id and self.shopify_country_id.id or False
            )
            ...
            values["shopify_pricelist_id"] = (
                self.shopify_pricelist_id and self.shopify_pricelist_id.id or False
            )
            values["shopify_stock_field"] = (
                self.shopify_stock_field and self.shopify_stock_field.id or False
            )
            ...
            values["shopify_section_id"] = (
                self.shopify_section_id and self.shopify_section_id.id or False
            )
            ...
            values["shopify_shipping_product_id"] = (
                self.shopify_shipping_product_id.id or False
            )
            values["gift_card_product_id"] = (
                self.shopify_gift_card_product_id.id or False
            )
            values["apply_tax_in_order"] = self.shopify_apply_tax_in_order
            values["invoice_tax_account_id"] = (
                self.shopify_invoice_tax_account_id
                and self.shopify_invoice_tax_account_id.id
                or False
            )
            values["credit_tax_account_id"] = (
                self.shopify_credit_tax_account_id
            ...
            values["shopify_activity_type_id"] = (
                self.shopify_activity_type_id
                and self.shopify_activity_type_id.id
                or False
            )  # add by bhavesh jadav 28/11/2019
            values["shopify_date_deadline"] = (
                self.shopify_date_deadline or False
            )  # add by bhavesh jadav 28/11/2019
            values.update(
                {"shopify_user_ids": [(6, 0, self.shopify_user_ids.ids)]}
            )  # Add by bhavesh jadav 28/11/2019
            values[
                "is_shopify_create_schedule"
            ] = self.is_shopify_create_schedule  # Add by bhavesh jadav 04/12/2019
            values["sync_product_with_images"] = (
                self.shopify_sync_product_with_images or False
            )  # Add by bhavesh jadav 17/12/2019 for images
            values[
                "create_shopify_products_webhook"
            ] = self.create_shopify_products_webhook
            values[
                "create_shopify_customers_webhook"
            ] = self.create_shopify_customers_webhook
            values["create_shopify_orders_webhook"] = self.create_shopify_orders_webhook
            values[
                "shopify_default_pos_customer_id"
            ] = self.shopify_default_pos_customer_id.id
            values["last_date_order_import"] = self.last_date_order_import
            values[
                "shopify_last_date_customer_import"
            ] = self.shopify_last_date_customer_import
            values[
                "shopify_last_date_update_stock"
            ] = self.shopify_last_date_update_stock
            values[
                "shopify_last_date_product_import"
            ] = self.shopify_last_date_product_import
            values["payout_last_import_date"] = (
                self.shopify_payout_last_date_import or False
            )
            values["shopify_settlement_report_journal_id"] = (
                self.shopify_settlement_report_journal_id or False
            )
            values["tip_product_id"] = (
                self.tip_product_id.id if self.tip_product_id else False
            )

            product_webhook_changed = (
                customer_webhook_changed
            ) = order_webhook_changed = False
            if (
                instance.create_shopify_products_webhook
                != self.create_shopify_products_webhook
            ):
            ...
            if (
                instance.create_shopify_orders_webhook
                != self.create_shopify_orders_webhook
            ):
    ...
    def onchange_shopify_company_id(self):
        ...
        domain = (
            self.shopify_company_id
            and [("company_id", "=", self.shopify_company_id.id)]
            or []
        )
