...
class ShopifyCronConfigurationEpt(models.TransientModel):
    ...
    shopify_instance_id = fields.Many2one(
        ...
        default=_get_shopify_instance,
        readonly=True,
    )

    # Auto cron for Export stock
    shopify_stock_auto_export = fields.Boolean(
        "Export Stock",
        default=False,
        help="Check if you want to automatically Export Stock levels from Odoo to Shopify.",
    )
    shopify_inventory_export_interval_number = fields.Integer(
        "Interval Number for Export stock", help="Repeat every x."
    )
    shopify_inventory_export_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
        ...
        "Interval Unit for Export Stock",
    )
    shopify_inventory_export_next_execution = fields.Datetime(
        "Next Execution for Export Stock ", help="Next Execution for Export Stock"
    )
    shopify_inventory_export_user_id = fields.Many2one(
        "res.users",
        string="User for Export Inventory",
        help="User for Export Inventory",
        default=lambda self: self.env.user,
    )

    # Auto cron for Import Order
    shopify_order_auto_import = fields.Boolean(
        "Import Order",
        default=False,
        help="Check if you want to automatically Import Orders from Shopify to Odoo.",
    )
    shopify_import_order_interval_number = fields.Integer(
        "Interval Number for Import Order", help="Repeat every x."
    )
    shopify_import_order_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
        ...
        "Interval Unit for Import Order",
    )
    shopify_import_order_next_execution = fields.Datetime(
        "Next Execution for Import Order", help="Next Execution for Import Order"
    )
    shopify_import_order_user_id = fields.Many2one(
        "res.users",
        string="User for Import Order",
        help="User for Import Order",
        default=lambda self: self.env.user,
    )

    # Auto cron for Update Order Status
    shopify_order_status_auto_update = fields.Boolean(
        "Update Order Status",
        default=False,
        help="Check if you want to automatically Update Order Status from Shopify to Odoo.",
    )
    shopify_order_status_interval_number = fields.Integer(
        "Interval Number for Update Order Status", help="Repeat every x."
    )
    shopify_order_status_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
    ...
    shopify_order_status_next_execution = fields.Datetime(
        "Next Execution for Update Order Status",
        help="Next Execution for Update Order Status",
    )
    shopify_order_status_user_id = fields.Many2one(
        "res.users",
        string="User for Update Order Status",
        help="User for Update Order Status",
        default=lambda self: self.env.user,
    )
    # Auto Import Payout Report
    shopify_auto_import_payout_report = fields.Boolean(
        string="Auto Import Payout Reports?"
    )
    shopify_payout_import_interval_number = fields.Integer(
        "Payout Import Interval Number", default=1, help="Repeat every x."
    )
    shopify_payout_import_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
        ...
        "Payout Import Interval Unit",
    )
    shopify_payout_import_next_execution = fields.Datetime(
        "Payout Import Next Execution", help="Next execution time"
    )
    shopify_payout_import_user_id = fields.Many2one(
        "res.users",
        string="Payout Import User",
        help="User",
        default=lambda self: self.env.user,
    )

    # Auto Generate Bank Statement
    shopify_auto_generate_bank_statement = fields.Boolean(
        string="Auto Generate Bank Statement?"
    )
    shopify_auto_generate_bank_statement_interval_number = fields.Integer(
        "Generate Bank Statement Interval Number", default=1, help="Repeat every x."
    )
    shopify_auto_generate_bank_statement_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
    ...
    shopify_auto_generate_bank_statement_next_execution = fields.Datetime(
        "Generate Bank Statement Next Execution", help="Next execution time"
    )
    shopify_auto_generate_bank_statement_user_id = fields.Many2one(
        "res.users",
        string="Generate Bank Statement User",
        help="User",
        default=lambda self: self.env.user,
    )

    # Auto Process Bank Statement
    shopify_auto_process_bank_statement = fields.Boolean(
        string="Auto Process Bank Statement?"
    )
    shopify_auto_process_bank_statement_interval_number = fields.Integer(
        "Process Bank Statement Interval Number", default=1, help="Repeat every x."
    )
    shopify_auto_process_bank_statement_interval_type = fields.Selection(
        [
            ("minutes", "Minutes"),
            ("hours", "Hours"),
            ("work_days", "Work Days"),
    ...
    shopify_auto_process_bank_statement_next_execution = fields.Datetime(
        "Process Bank Statement " "Next Execution", help="Next execution time"
    )
    shopify_auto_process_bank_statement_user_id = fields.Many2one(
        "res.users",
        string="Process Bank Statement User",
        help="User",
        default=lambda self: self.env.user,
    )
    ...
    def update_export_stock_cron_field(self, instance):
        ...
        try:
            export_inventory_stock_cron_exist = instance and self.env.ref(
                "shopify_ept.ir_cron_shopify_auto_export_inventory_instance_%d"
                % (instance.id)
            )
        except:
            export_inventory_stock_cron_exist = False
        if export_inventory_stock_cron_exist:
            self.shopify_stock_auto_export = (
                export_inventory_stock_cron_exist.active or False
            )
            self.shopify_inventory_export_interval_number = (
                export_inventory_stock_cron_exist.interval_number or False
            )
            self.shopify_inventory_export_interval_type = (
                export_inventory_stock_cron_exist.interval_type or False
            )
            self.shopify_inventory_export_next_execution = (
                export_inventory_stock_cron_exist.nextcall or False
            )
            self.shopify_inventory_export_user_id = (
                export_inventory_stock_cron_exist.user_id.id or False
            )
    ...
    def update_import_order_cron_field(self, instance):
        ...
        try:
            import_order_cron_exist = instance and self.env.ref(
                "shopify_ept.ir_cron_shopify_auto_import_order_instance_%d"
                % (instance.id)
            )
        except:
        ...
        if import_order_cron_exist:
            ...
            self.shopify_import_order_interval_number = (
                import_order_cron_exist.interval_number or False
            )
            self.shopify_import_order_interval_type = (
                import_order_cron_exist.interval_type or False
            )
            self.shopify_import_order_next_execution = (
                import_order_cron_exist.nextcall or False
            )
            self.shopify_import_order_user_id = (
                import_order_cron_exist.user_id.id or False
            )
    ...
    def update_order_status_cron_field(self, instance):
        ...
        if update_order_status_cron_exist:
            self.shopify_order_status_auto_update = (
                update_order_status_cron_exist.active or False
            )
            self.shopify_order_status_interval_number = (
                update_order_status_cron_exist.interval_number or False
            )
            self.shopify_order_status_interval_type = (
                update_order_status_cron_exist.interval_type or False
            )
            self.shopify_order_status_next_execution = (
                update_order_status_cron_exist.nextcall or False
            )
            self.shopify_order_status_user_id = (
                update_order_status_cron_exist.user_id.id or False
            )
    ...
    def update_payout_report_cron_field(self, instance):
        ...
        try:
            payout_report_cron_exist = instance and self.env.ref(
                "shopify_ept.ir_cron_auto_import_payout_report_instance_%d"
                % (instance.id)
            )
        except:
        ...
        try:
            auto_generate_bank_statement_cron_exist = instance and self.env.ref(
                "shopify_ept.ir_cron_auto_generate_bank_statement_instance_%d"
                % (instance.id)
            )
        except:
        ...
        try:
            auto_process_bank_statement_cron_exist = instance and self.env.ref(
                "shopify_ept.ir_cron_auto_process_bank_statement_instance_%d"
                % (instance.id)
            )
        except:
        ...
        if payout_report_cron_exist and payout_report_cron_exist.active:
            ...
            self.shopify_payout_import_interval_number = (
                payout_report_cron_exist.interval_number or False
            )
            self.shopify_payout_import_interval_type = (
                payout_report_cron_exist.interval_type or False
            )
            self.shopify_payout_import_next_execution = (
                payout_report_cron_exist.nextcall or False
            )
            self.shopify_payout_import_user_id = (
                payout_report_cron_exist.user_id.id or False
            )
        if (
            auto_generate_bank_statement_cron_exist
            and auto_generate_bank_statement_cron_exist.active
        ):
            self.shopify_auto_generate_bank_statement = (
                auto_generate_bank_statement_cron_exist.active
            )
            self.shopify_auto_generate_bank_statement_interval_number = (
                auto_generate_bank_statement_cron_exist.interval_number or False
            )
            self.shopify_auto_generate_bank_statement_interval_type = (
                auto_generate_bank_statement_cron_exist.interval_type or False
            )
            self.shopify_auto_generate_bank_statement_next_execution = (
                auto_generate_bank_statement_cron_exist.nextcall or False
            )
            self.shopify_auto_generate_bank_statement_user_id = (
                auto_generate_bank_statement_cron_exist.user_id.id or False
            )
        if (
            auto_process_bank_statement_cron_exist
            and auto_process_bank_statement_cron_exist.active
        ):
            self.shopify_auto_process_bank_statement = (
                auto_process_bank_statement_cron_exist.active
            )
            self.shopify_auto_process_bank_statement_interval_number = (
                auto_process_bank_statement_cron_exist.interval_number or False
            )
            self.shopify_auto_process_bank_statement_interval_type = (
                auto_process_bank_statement_cron_exist.interval_type or False
            )
            self.shopify_auto_process_bank_statement_next_execution = (
                auto_process_bank_statement_cron_exist.nextcall or False
            )
            self.shopify_auto_process_bank_statement_user_id = (
                auto_process_bank_statement_cron_exist.user_id.id or False
            )
    ...
    def setup_shopify_inventory_export_cron(self, instance):
        ...
        if self.shopify_stock_auto_export:
            ...
            vals = {
                ...
                "nextcall": self.shopify_inventory_export_next_execution
                or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
                "code": "model.update_stock_in_shopify(ctx={'shopify_instance_id':%d})"
                % (instance.id),
                "user_id": self.shopify_inventory_export_user_id
                and self.shopify_inventory_export_user_id.id,
            }
            if cron_exist:
    ...
    def setup_shopify_import_order_cron(self, instance):
        ...
        if self.shopify_order_auto_import:
            ...
            vals = {
                ...
                "nextcall": self.shopify_import_order_next_execution
                or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
                "code": "model.import_order_cron_action(ctx={'shopify_instance_id':%d})"
                % (instance.id),
                "user_id": self.shopify_import_order_user_id
                and self.shopify_import_order_user_id.id,
            }
            if cron_exist:
                vals.update({"name": cron_exist.name})
                cron_exist.write(vals)
                ...
                try:
                    import_order_cron = self.env.ref(
                        "shopify_ept.ir_cron_shopify_auto_import_order"
                    )
                except:
    ...
    def setup_shopify_update_order_status_cron(self, instance):
        ...
        if self.shopify_order_status_auto_update:
            ...
            vals = {
                ...
                "nextcall": self.shopify_order_status_next_execution
                or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
                "code": "model.update_order_status_cron_action(ctx={'shopify_instance_id':%d})"
                % (instance.id),
                "user_id": self.shopify_order_status_user_id
                and self.shopify_order_status_user_id.id,
            }
            if cron_exist:
                vals.update({"name": cron_exist.name})
                cron_exist.write(vals)
                ...
                try:
                    update_order_status_cron = self.env.ref(
                        "shopify_ept.ir_cron_shopify_auto_update_order_status"
                    )
                except:
    ...
    def setup_shopify_payout_auto_import_payout_report_cron(self, instance):
        ...
        vals = {
            ...
            "nextcall": self.shopify_payout_import_next_execution
            or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
            "code": "model.auto_import_payout_report(ctx={'shopify_instance_id':%d})"
            % (instance.id),
            "user_id": self.shopify_payout_import_user_id
            and self.shopify_payout_import_user_id.id,
        }
        ...
        if cron_exist:
            ...
            try:
                update_order_cron = self.env.ref(
                    "shopify_ept.ir_cron_auto_import_payout_report"
                )
            except:
    ...
    def setup_shopify_payout_auto_generate_bank_statement_cron(self, instance):
        ...
        vals = {
            ...
            "nextcall": self.shopify_auto_generate_bank_statement_next_execution
            or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
            "code": "model.auto_generate_bank_statement(ctx={'shopify_instance_id':%d})"
            % (instance.id),
            "user_id": self.shopify_auto_generate_bank_statement_user_id
            and self.shopify_auto_generate_bank_statement_user_id.id,
        }
        ...
        if cron_exist:
            ...
            try:
                update_cron = self.env.ref(
                    "shopify_ept.ir_cron_auto_generate_bank_statement"
                )
            except:
                update_cron = False
            if not update_cron:
    ...
    def setup_shopify_payout_auto_process_bank_statement_cron(self, instance):
        ...
        vals = {
            ...
            "nextcall": self.shopify_auto_process_bank_statement_next_execution
            or nextcall.strftime("%Y-%m-%d %H:%M:%S"),
            "code": "model.auto_process_bank_statement(ctx={'shopify_instance_id':%d})"
            % (instance.id),
            "user_id": self.shopify_auto_process_bank_statement_user_id
            and self.shopify_auto_process_bank_statement_user_id.id,
        }
        if cron_exist:
            vals.update({"name": cron_exist.name})
            cron_exist.write(vals)
            ...
            try:
                update_cron = self.env.ref(
                    "shopify_ept.ir_cron_auto_process_bank_statement"
                )
            except:
                update_cron = False
            if not update_cron:
