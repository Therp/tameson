...
class PrepareProductForExport(models.TransientModel):
    ...
    export_method = fields.Selection(
        [
            ("direct", "Export in Odoo Shopify Product List"),
    ...
    def export_direct_in_shopify(self, product_templates):
        ...
        for variant in variants:
            ...
            (
                ...
                sequence,
                shopify_template_id,
            ) = self.create_or_update_shopify_layer_template(
                shopify_instance,
                product_template,
                variant,
                shopify_template_id,
            ...
            if (
                shopify_template
                and shopify_template.shopify_product_ids
                and shopify_template.shopify_product_ids[0].sequence
            ):
                sequence += 1

            shopify_variant = self.create_or_update_shopify_layer_variant(
                variant,
                shopify_template_id,
                shopify_instance,
                shopify_template,
    ...
    def create_or_update_shopify_layer_template(
        self, shopify_instance, product_template, variant, shopify_template_id, sequence
    ):
        ...
        Task_id: 181893 - Shopify fixes as per the new version
        """
        shopify_templates = shopify_template_obj = self.env[
        ...
        if not shopify_template:
            shopify_product_template_vals = (
                self.prepare_template_val_for_export_product_in_layer(
                    product_template, shopify_instance, variant
                )
            )
    ...
    def prepare_template_val_for_export_product_in_layer(
        self, product_template, shopify_instance, variant
    ):
    ...
    def prepare_variant_val_for_export_product_in_layer(
        self, shopify_instance, shopify_template, variant, sequence
    ):
    ...
    def create_or_update_shopify_layer_variant(
        self, variant, shopify_template_id, shopify_instance, shopify_template, sequence
    ):
        ...
        shopify_variant_vals = self.prepare_variant_val_for_export_product_in_layer(
            shopify_instance, shopify_template, variant, sequence
        )
    ...
    def export_xlsx_file(self, product_templates):
        ...
        self.write({"choose_file": b_data, "file_name": "Shopify_export_product_"})
        return {
            "type": "ir.actions.act_url",
            "url": "web/content/?model=shopify.prepare.product.for.export.ept&id=%s&field=choose_file&download=true&"
            "filename=%s.xlsx"
    ...
    def create_shopify_template_images(self, shopify_template):
        ...
        for odoo_image in product_template.ept_image_ids.filtered(
    ...
    def import_products_from_file(self):
        ...
        """
        try:
            if os.path.splitext(self.file_name)[1].lower() not in [
                ".csv",
                ".xls",
                ".xlsx",
            ]:
                raise UserError(
                    _(
                        "Invalid file format. You are only allowed to upload .csv, .xlsx file."
                    )
                )
    ...
    def create_products_from_file(self, file_data):
        ...
        Task_id: 181893 - Shopify fixes as per the new version
        """
        prepare_product_for_export_obj = self.env[
            "shopify.prepare.product.for.export.ept"
        ]
        ...
        log_book_id = common_log_obj.create(
            {
                "type": "import",
                "module": "shopify_ept",
        ...
        for record in file_data:
            ...
            message = ""
            if (
                not record["PRODUCT_TEMPLATE_ID"]
                or not record["PRODUCT_ID"]
                or not record["CATEGORY_ID"]
            ):
                message += (
                    "PRODUCT_TEMPLATE_ID Or PRODUCT_ID Or CATEGORY_ID Not As Per Odoo Product in file at row "
                    "%s " % row_no
                )
                vals = {
                    "message": message,
            ...
            (
                ...
                shopify_template_id,
                sequence,
            ) = self.create_or_update_shopify_template_from_csv(
                instance, record, shopify_template_id, sequence
            )

            shopify_variant = self.create_or_update_shopify_variant_from_csv(
                instance, record, shopify_template_id, sequence
            )
            prepare_product_for_export_obj.create_shopify_variant_images(
                shopify_template, shopify_variant
            )
    ...
    def create_or_update_shopify_template_from_csv(
        self, instance, record, shopify_template_id, sequence
    ):
        ...
        if (
            ...
            .sudo()
            .get_param("shopify_ept.set_sales_description")
        ):
            shopify_product_template_vals.update(
                {"description": record["product_description"]}
            )
        if not shopify_template:
            shopify_template = shopify_product_template.create(
                shopify_product_template_vals
            )
            sequence = 1
            shopify_template_id = shopify_template.id
    ...
    def create_or_update_shopify_variant_from_csv(
        self, instance, record, shopify_template_id, sequence
    ):
    ...
    def read_xlsx_file(self):
        ...
        validation_header = []
        product_data = []
        sheets = xlrd.open_workbook(
            file_contents=base64.b64decode(self.choose_file.decode("UTF-8"))
        )
        header = dict()
        is_header = False
        for sheet in sheets.sheets():
            for row_no in range(sheet.nrows):
                ...
                row = dict()
                [
                    row.update({k: sheet.row(row_no)[v].value})
                    for k, v in header.items()
                    for c in sheet.row(row_no)
                ]
