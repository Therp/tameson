...
class ShopifyQueueProcessEpt(models.TransientModel):
    ...
    def process_customer_queue_manually(self):
        ...
        if customer_queue_id.synced_customer_queue_line_ids:
            synced_customer_queue_line_ids = (
                customer_queue_id.synced_customer_queue_line_ids.filtered(
                    lambda line: line.state == "draft"
                )
            )
            if synced_customer_queue_line_ids:
                customer_line_obj.sync_shopify_customer_into_odoo(
                    synced_customer_queue_line_ids
                )
    ...
    def process_order_queue_manually(self):
        ...
        """
        self.env["shopify.order.data.queue.line.ept"]
        ...
        for order_queue_id in order_queue_ids:
            order_queue = shopify_order_queue_obj.browse(order_queue_id)
            order_queue_lines = order_queue.order_data_queue_line_ids.filtered(
                lambda x: x.state in ["draft", "failed"]
            )

            if order_queue.created_by == "webhook":
                order_queue_lines.process_import_order_queue_data(update_order=True)
            else:
    ...
    def set_to_completed_order_queue_manually(self):
        ...
        """
        order_queue_ids = self._context.get("active_ids")
        order_queue_ids = self.env["shopify.order.data.queue.ept"].browse(
            order_queue_ids
        )
        for order_queue_id in order_queue_ids:
            ...
            order_queue_id.message_post(
                body=_("Manually set to cancel queue lines %s - ")
    ...
    def set_to_completed_product_queue_manually(self):
        ...
        """
        product_queue_ids = self._context.get("active_ids")
        product_queue_ids = self.env["shopify.product.data.queue.ept"].browse(
            product_queue_ids
        )
        for product_queue_id in product_queue_ids:
    ...
    def set_to_completed_customer_queue_manually(self):
        ...
        customer_queue_ids = self.env["shopify.customer.data.queue.ept"].browse(
            customer_queue_ids
        )
    ...
    def instance_active_archive(self):
        """This method is used to active/inactive instance.
        ...
        """
        instances = self.env["shopify.instance.ept"].browse(
