...
SERIALIZERS = [
    {"type": bool, "method": lambda value: ("boolean", six.text_type(value).lower())},
    {
...
if six.PY2:
    SERIALIZERS.append(
        {"type": str, "method": lambda value: (None, unicode(value, "utf-8"))}
    )
else:
    SERIALIZERS.append(
        {
...
class FileObject(object):
    ...
    def __init__(self, data, name="untitled", content_type="application/octet-stream"):
        self.data = data
        self.name = name
...
def xml_to_dict(xmlobj, saveroot=True):
    ...
    if element_type == "array":
        ...
        if saveroot:
            return element_containers.ElementDict(
                element_list_type, {element_list_type: return_list}
            )
        else:
            return return_list
        ...
        element_type in ("integer", "datetime", "date", "decimal", "double", "float")
        ...
        if date_parse:
            ...
            try:
                ...
                raise Error(
                    "Unable to parse timestamp. Install dateutil"
        ...
        content_type = element.get("content_type", "application/octet-stream")
        ...
        if element_type:
            ...
            attributes = element_containers.ElementDict(
                singularize(element.tag.replace("-", "_")), element.items()
            )
        for child in element.getchildren():
            ...
            if child_tag in attributes:
                if isinstance(attributes[child_tag], list):
                    ...
                    attributes[child_tag] = [attributes[child_tag], attribute]
            else:
