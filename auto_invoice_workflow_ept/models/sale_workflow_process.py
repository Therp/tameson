...
class SaleWorkflowProcess(models.Model):
    ...
    invoice_date_is_order_date = fields.Boolean(
        ...
        help="If it's check the Accounting Date will be the " "same as the order date",
    )
    ...
    def shipped_order_workflow(self, orders, customers_location):
        ...
        mrp_module = module_obj.sudo().search(
        ...
        for order in orders:
            if order.order_line:
                ...
                delivered_lines = order.mapped("order_line").filtered(
                    lambda l: l.product_id.invoice_policy != "order"
                )
                if (
