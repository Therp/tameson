...
class SaleOrder(models.Model):
    ...
    auto_workflow_process_id = fields.Many2one(
        "sale.workflow.process.ept", string="Workflow Process", copy=False
    )
    ...
    def auto_shipped_order_ept(self, customers_location, is_mrp_installed=False):
        ...
        for order_line in order_lines:
            if not is_mrp_installed:
                if order_line.product_id.is_drop_ship_product:
                    self.create_and_done_stock_move(
                        order_line, customers_location, vendor_location=vendor_location
                    )
                else:
                ...
                for bom_line in bom_lines:
                    self.create_and_done_stock_move(
                        order_line, customers_location, bom_line
                    )
    ...
    def create_and_done_stock_move(
        self, order_line, customers_location, bom_line=False, vendor_location=False
    ):
        """
        Added by Udit
        ...
        if product and product_qty and product_uom:
            vals = {
                ...
                if vendor_location
                else self.warehouse_id.lot_stock_id.id,
