...
class TamesonMergeContact(models.TransientModel):
    ...
    line_ids = fields.One2many(
        comodel_name="tameson.merge.contact.line",
        ...
        for line in self.line_ids:
            ...
            if len(ids) > 1:
                wiz = (
                    ...
                    .with_context(active_ids=ids, active_model="res.partner")
                    .create({})
                )
                ...
                try:
                    wiz.with_context(
                        skip_email_check=True, skip_child_check=True
                    ).action_merge()
            ...
            last_order = self.env["sale.order"].search(
                [("state", "=", "sale"), ("partner_id", "=", partner.id)],
            ...
            if not last_order:
                del_inv += partner.child_ids.filtered(
            ...
            for child in (
                partner.child_ids.filtered(lambda p: p.type != "contact") - del_inv
            ):
                if (child.zip or "").lower().strip() not in (del_inv + other).mapped(
                    lambda p: (p.zip or "").lower().strip()
                ):
                    other += child
                else:
                    arch += child
...
class TamesonMergeContactLine(models.TransientModel):
    ...
    merge_id = fields.Many2one(
        comodel_name="tameson.merge.contact",
        ondelete="cascade",
    )
